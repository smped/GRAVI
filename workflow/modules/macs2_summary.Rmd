...
## Should probably export after mapping...?
```
...
msigdb <- read_rds(all_paths$annotation, "msigdb.rds")
gs_url <- unique(msigdb$gs_url)
```
...
A simple enrichment analysis was performed by selecting a set of `r comma(motif_enrich_res$sequences[[1]] * motif_enrich_res$n_iter[[1]])` *background sequences* matched by position with genomic regions to the `r comma(motif_enrich_res$sequences[[1]])` *test sequences*.
