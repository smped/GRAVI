...
peak_path <- here::here("output", "peak_analysis", target)
...
filtered_peaks <- peak_path %>% 
  file.path("{target}_{treat_levels}_filtered_peaks.narrowPeak") %>% 
  glue() %>% 
  importPeaks(seqinfo = sq) %>% 
  setNames(treat_levels)
consensus_peaks <- peak_path %>% 
  file.path("{target}_consensus_peaks.bed.gz") %>% 
  glue() %>% 
...
mlz <- peak_path %>% 
  file.path("{target}_regions_localz.rds") %>% 
  glue() %>% 
  read_rds()
...
motif_list <- file.path(annotation_path, "motif_list.rds") %>% 
  ...
  to_df %>% 
  as_tibble()
pos_res <- file.path(peak_path, "{target}_motif_position.tsv.gz") %>% 
  glue() %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_pos <- sum(pos_res$adj_p < motif_params$analysis$alpha)
enrich_res <- peak_path %>% 
  file.path("{target}_motif_enrichment.tsv.gz") %>%
  glue() %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_enrich <- sum(enrich_res$adj_p < motif_params$analysis$alpha & enrich_res$Z > 0)
matches <- file.path(peak_path, "{target}_matches.rds") %>% 
  glue() %>% 
  read_rds()
```
