...
macs2_path <- here::here(params$macs2_path)
peak_path <- here::here(params$peak_path)
...
macs2_params <- here::here("config", "json", "macs2_param.json") %>%
  jsonlite::fromJSON() %>%
  pluck(target)
macs2_qc_params <- here::here("config", "json", "macs2_qc_param.json") %>%
  jsonlite::fromJSON() %>%
  pluck(target)
    ...
    caption = glue(
      ...
      ifelse(
        ...
        glue(
          ...
          "+/- {macs2_qc_params$outlier_threshold}-fold the median number of ",
...
Peaks detected when merging samples, and which overlapped a peak in more than `r percent(macs2_qc_params$min_prop_reps)` of the *individual replicates passing QC* in each treatment group, were retained.
...
treat_levels %>% 
  ...
  pander(
    ...
    caption = glue(
      ...
      "{percent(macs2_qc_params$min_prop_reps)} of the qc-passing samples for each treatment group,",
      "as described above.",
      .sep = " "
    )
  )
...
The `r length(gs_list)` pathways tested were those belonging to `r pander(unname(unlist(msigdb_params[c("gs_cat", "gs_subcat")])))` as defined by [MSigDB](https://www.gsea-msigdb.org/gsea/msigdb/) v`r packageVersion("msigdbr")` [@Liberzon2011-fr].
