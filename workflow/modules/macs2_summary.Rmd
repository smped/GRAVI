...
The `r length(gs_list)` pathways tested were `r pander(unname(unlist(extra_params$enrichment$msigdb)))` as defined by MSigDB v`r packageVersion("msigdbr")`.
Pathways will be considered as enriched relative to background if returned an adjusted p-value (`r extra_params$enrichment$adj`) below `r extra_params$enrichment$alpha`, using the `rGREAT` results from the *hyper-geometric test*, instead of the *binomial test*.

```{r run-great}
great_res <- great(
    set_genome_info(
    consensus_peaks, genome = map_great_refs(config$genome$build)
  ), 
...
great_df <- getEnrichmentTable(great_res) %>% 
  ...
  mutate(p_adjust_hyper = p.adjust(p_value_hyper, adj_method)) %>% 
