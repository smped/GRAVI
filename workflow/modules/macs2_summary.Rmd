    ...
    caption = glue(
      ...
      ifelse(
        ...
        glue(
          ...
          "+/- {config$macs2_qc$default$outlier_threshold}-fold the median number of ",
  ...
  sapply(
    function(x) {
      gr <- macs2_path %>%
        ...
        unlist()
      k <- dplyr::filter(n_reps, treat == x)$n * config$macs2_qc$default$$min_prop_reps
      if (k > 0) {
...
Treatment peaks which overlapped a peak in more than `r percent(config$macs2_qc$default$min_prop_reps)` of the *individual replicates passing QC* in each treatment group, were retained.
...
treatment_peaks %>%
  ...
  pander(
    ...
    caption = glue(
      ...
      "{percent(config$macs2_qc$default$min_prop_reps)} of the retained samples for each", "treatment group, as described above.",
      .sep = " "
    )
  )
```
