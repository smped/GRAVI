...
```{r plot-dist-to-tss-hist, fig.height=6, fig.cap = "Distances from the centre of the consensus peak to the transcription start-site shown as A) a histogram, and B) as a cumulative distribution. The range is restricted to +/-100kb around the TSS in both cases"}
tss <- resize(all_gr$transcript, width = 1) %>% 
  setNames(.$transcript_id)
a <- consensus_peaks %>% 
  join_nearest(tss) %>% 
  mutate(
    tss_start = start(tss[transcript_id]),
    d = start + width/2 - tss_start
  ) %>% 
  mcols() %>% 
  as.data.frame() %>%
  ggplot(aes(d / 1e3)) +
...
## Promoter: 1) <2kb around TSS, 2) <5kb upstream
## Gene Body: 1) Exon; 2) 1st Intron; 3) Other Intron
## Intergenic: 1) 5-10kb; 2) 10-100kb; 3) Distal
