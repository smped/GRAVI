...
```{r plot-common-peaks, results = 'hide'}
...
```{r plot-common-upset, eval = length(treat_levels) > 3, echo = length(treat_levels) > 3, fig.height = 7, fig.cap = "*Number of consensus peaks which overlap the oracle peaks defined within each condition.*"}
oracle_peaks %>%
  lapply(
    function(x) {
      names(subsetByOverlaps(consensus_peaks, x))
