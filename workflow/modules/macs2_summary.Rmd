...
gtf_transcript <- read_rds(file.path(all_paths$annotation, "gtf_transcript.rds"))
...
motif_uri <- file.path(all_paths$annotation, "motif_uri.rds") %>% 
  read_rds()
pos_res <- file.path(all_paths$peaks, "{target}_motif_position.tsv.gz") %>% 
  glue() %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_pos <- sum(pos_res$adj_p < motif_params$alpha)
enrich_res <- all_paths$peaks %>% 
