...
network_params <- here::here("config", "params.yml") %>% 
  read_yaml() %>% 
  pluck("networks")
motif_params <- here::here("config", "params.yml") %>% 
  read_yaml() %>% 
  pluck("motif_analysis")
```
...
## External features
external_features <- read_rds(file.path(annotation_path, "features.rds"))
...
if (has_features) {
  consensus_peaks$Feature <- bestOverlap(
    consensus_peaks, external_features, missing = "no_feature"
  ) %>%
...
min_network_size <- c(network_params$min_size, 2)[[1]]
max_network_size <- c(network_params$max_size, Inf)[[1]]
...
msigdb <- msigdbr(enrich_params$species) %>% 
  dplyr::filter(
...
cp <- htmltools::em(
  glue(
    ...
    most highly ranked. For gene-sets where rGREAT has returned a p-value of 
    zero, the value <2e-16 is printed.
    "
  )
)
div(class = "table",
...
mlz <- here::here(macs2_path, target, glue("{target}_regions_localz.rds")) %>% 
  read_rds()
...
fw_peaks <- resize(
  consensus_peaks, width = motif_params$analysis$peak_width, fix = "center"
)
genome(fw_peaks) <- ucsc$build
fw_seq <- getSeq(bs_genome, fw_peaks)
motif_adj <- match.arg(motif_params$analysis$adj, p.adjust.methods)
motif_list <- file.path(annotation_path, "motif_list.rds") %>% 
  read_rds() %>% 
  to_df %>% 
  as_tibble()
res_pos <- file.path(macs2_path, target, "{target}_motif_position.tsv.gz") %>% 
  glue() %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
res_enrich <- macs2_path %>% 
  file.path(target, "{target}_motif_enrichment.tsv.gz") %>%
  glue() %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_enrich <- sum(res_enrich$adj_p < motif_params$analysis$alpha & res_enrich$Z > 0)
matches <- file.path(macs2_path, target, "{target}_matches.rds") %>% 
  glue() %>% 
  read_rds()
```
...
For this step peaks were set to a fixed width of `r motif_params$analysis$peak_width`bp, with sequences taken from these regions.

### Motif Enrichment {.tabset}

A simple enrichment analysis was performed by selecting a set of `r comma(res_enrich$sequences[[1]] * res_enrich$n_iter[[1]])` *background sequences* matched by position with genomic regions to the `r comma(res_enrich$sequences[[1]])` *test sequences*.
This gave a set of genomically matched control sequences to gauge enrichment by.
Analysis was performed using the `motifTestR` package and the `r motif_params$analysis$model`.
...
tbl <- res_enrich %>% 
  arrange(p) %>% 
  dplyr::filter(adj_p < motif_params$analysis$alpha, Z > 0) %>% 
  mutate(cluster = fct_inorder(as.character(cluster))) %>% 
  dplyr::select(
    cluster, ends_with("name"), Z, adj_p, matches, expected, enrichment
  ) %>% 
  mutate(PWM = altname) %>% 
  reactable(
    groupBy = "cluster",
    ...
    columns = list(
      cluster = colDef("Cluster", maxWidth = 70, filterable = TRUE),
      altname = colDef(
        "Motif", minWidth = 180, aggregate = "unique", filterable = TRUE
      ),
      name = colDef(
        "Name", minWidth = 120, aggregate = "unique", filterable = TRUE
      ),
      matches = colDef(
      ...
      adj_p = colDef(
        ...
        format = colFormat(digits = 3), filterable = TRUE,
        filterMethod = JS("function(rows, columnId, filterValue) {
          return rows.filter(function(row) {
            return row.values[columnId] <= filterValue
          })
      ...
      PWM = colDef(
        name = "IC Matrix", minWidth = 180,
        cell = function(value) {
          img_src <- knitr::image_uri(
            file.path("assets", "motifs", paste0(value, ".png"))
          )
          tagList(
            tags$img(src = img_src, height = '80px')
          )
        }
      )
    )
...
cp <- htmltools::em(
  glue(
    ...
    of sequences, using the {motif_params$analysis$model} model and an adjusted 
    p-value ({motif_adj}) < {motif_params$analysis$alpha} as the criteria for 
...
div(class = "enriched-motifs",
    div(class = "enriched-motifs-caption",
        div(class = "caption", cp), tbl
    )
)
```

#### Comparison of Motif Co-Occurrence

```{r enrich-corrplot, fig.width=8, fig.cap = glue("*The top {length(top_enrich)} motifs ranked for enrichment were compared to each other using correlations derived from simple presence/absence calls. Highly correlated motifs may indicate similarity between TFBMs and the same sequence match being identified*")}
top_enrich <- res_enrich %>%
  dplyr::filter(Z > 0, adj_p < motif_params$analysis$alpha) %>% 
  dplyr::slice(1:20) %>% 
  pull("altname")
matches[top_enrich] %>% 
  lapply(\(x) tibble(seq = x$seq)) %>% 
  bind_rows(.id = "altname") %>%
  distinct(altname, seq) %>% 
  left_join(
    dplyr::select(res_enrich, altname, name), by = "altname"
  ) %>% 
...
Positional bias was tested using the package `motifTestR`, bin-sizes of `r motif_params$analysis$binwidth`bp and testing `r ifelse(motif_params$analysis$abs, "distance from sequence centres", "position either side of the sequence centres")`.
Any motifs which matched fewer than `r percent(motif_params$analysis$ignore_below)` of sequences were not tested for any positional bias.

Using an adjusted p-value (`r motif_adj`) less than `r motif_params$analysis$alpha` to be considered as significant, `r sum(res_pos$adj_p < motif_params$analysis$alpha)` motifs were considered as showing any type of positional bias.
...
tbl <- res_pos %>%
  ...
  mutate(PWM = altname) %>% 
  reactable(
    groupBy = "cluster", showPageSizeOptions = TRUE,
    columns = list(
      cluster = colDef(maxWidth = 60, filterable = TRUE),
      name = colDef(
        "Name", aggregate = "unique", maxWidth = 80, filterable = TRUE
      ),
      altname = colDef(
        "Motif", aggregate = "unique", minWidth = 180, filterable = TRUE
      ),
      start = colDef(
        "Start", aggregate = "min", maxWidth = 50, filterable = TRUE,
        filterMethod = JS("function(rows, columnId, filterValue) {
          return rows.filter(function(row) {
            return row.values[columnId] >= filterValue
          })
        ...
        "End", aggregate = "max", maxWidth = 50, filterable = TRUE,
        filterMethod = JS("function(rows, columnId, filterValue) {
          return rows.filter(function(row) {
            return row.values[columnId] >= filterValue
          })
      ...
      width = colDef(
        "Width", aggregate = "median", maxWidth = 60, filterable = TRUE,
        filterMethod = JS("function(rows, columnId, filterValue) {
          return rows.filter(function(row) {
            return row.values[columnId] <= filterValue
          })
      ...
      matches_in_region = colDef(
        "Region", aggregate = "max", maxWidth = 60, filterable = TRUE,
        filterMethod = JS("function(rows, columnId, filterValue) {
          return rows.filter(function(row) {
            return row.values[columnId] >= filterValue
          })
...
cp <- htmltools::em(
  glue(
    "
    All {sum(res_pos$adj_p < motif_params$analysis$alpha)} TFBMs found to show
    positional bias using an adjusted p-value ({motif_adj}) below
    {motif_params$analysis$alpha} as the criteria for significance. Highly 
...
div(class = "positional-motifs",
    div(class = "positional-motifs-caption",
        div(class = "caption", cp), tbl
    )
)
```
...
```{r plot-total-pos-matches, fig.cap = glue("*All best matches by their distance from the sequence centre, without binning by position. Black lines represent the loess curve through all matches. The motif with the most matches within the enriched region is shown for each cluster, with the other motifs within the cluster indicated below in brackets.*")}
lb <- glue("{names(clust_names)}\n({clust_names})") %>% 
  setNames(names(clust_names))
matches[top_pos$altname] %>% 
  plotMatchPos(
    geom = "point", abs = motif_params$analysis$abs, use_totals = TRUE,
    binwidth = 1
  ) +
...
```{r plot-smoothed-pos, fig.height=6, fig.cap = glue("*Best matches for the representative motif within each cluster, shown as (A) proportions and (B) as a cumulative distribution*")}
A <- matches[top_pos$altname] %>% 
  plotMatchPos(
    se = FALSE, abs = motif_params$analysis$abs, linewidth = 1/2,
    binwidth = motif_params$analysis$binwidth / 2
  ) +
  labs(
    x = "Distance From Centre", y = "Proportion", colour = "Motifs In Cluster"
  ) +
  theme(legend.position = "none")
B <- matches[top_pos$altname] %>% 
  plotMatchPos(
    type = "cdf", geom = "line", abs = motif_params$analysis$abs,
...
```{r heatmap-pos, fig.cap = glue("*The top-ranked motifs by positional bias showing the total number of matches within each bin. Motifs are clustered by similarity of totals within this figure.*")}
top_pos <- res_pos %>% 
  dplyr::filter(adj_p < motif_params$analysis$alpha) %>% 
  dplyr::slice(1:25) %>% 
  pull(altname)
matches[top_pos] %>% 
  plotMatchPos(
    abs = motif_params$analysis$abs, type = "heatmap", cluster = TRUE,
    use_totals = TRUE,
    # binwidth = floor(motif_params$analysis$binwidth / 2)
    binwidth = motif_params$analysis$binwidth 
