...
great_res <- consensus_peaks %>% 
  set_genome_info(genome = ucsc_ref) %>% 
  great(
...
motif_list <- file.path(annotation_path, "motif_list.rds") %>% 
  ...
  as_tibble()
res_pos <- file.path(
  macs2_path, target, glue("{target}_motif_position.tsv.gz")
) %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
# res_enrich <- file.path(
#   macs2_path, target, glue("{target}_motif_enrichment.tsv.gz")
# ) %>% 
#   read_tsv()
```
...
Positional bias was tested using the packabe `motifTestR`, bin-sizes of `r motif_params$analysis$binwidth`bp and testing `r ifelse(motif_params$analysis$abs, "distance from sequence centres", "position either side of the sequence centres")`.
...
top_pos <- res_pos %>% 
  ...
  dplyr::filter(enrichment == max(enrichment), .by = cluster) %>% 
  dplyr::slice(1:9) %>% 
...
bm_pos <- motif_list %>% 
  ...
  to_list() %>% 
  getPwmMatches(fw_seq, best_only = TRUE, mc.cores = threads) %>% 
  .[top_pos$altname] 
```
...
res_pos %>% 
  ...
  reactable(
    ...
    columns = list(
      ...
      name = colDef("Name", aggregate = "unique", maxWidth = 80, filterable = TRUE),
      altname = colDef("Motif", aggregate = "unique", minWidth = 180, filterable = TRUE),
      start = colDef("Start", aggregate = "min", maxWidth = 50),
      end = colDef("End", aggregate = "max", maxWidth = 50),
      centre = colDef("Centre", aggregate = "median", maxWidth = 60),
      width = colDef("Width", aggregate = "median", maxWidth = 50),
      total_matches = colDef(
        "Total Matches", aggregate = "max", maxWidth = 60, format = comma_col
      ),
      matches_in_region = colDef(
        "Region Matches", aggregate = "max", maxWidth = 60
      ),
      expected = colDef(
        "Expected", aggregate = "max", maxWidth = 70, format = comma_col
      ),
      enrichment = colDef(
        "Enrichment", aggregate = "max", format = colFormat(digits = 2),
        maxWidth = 80
      ),
      prop_total = colDef(
...
```{r}
 plotMatchPos(
  bm_pos, geom = "point", abs = motif_params$analysis$abs,
  binwidth = 1#motif_params$analysis$binwidth
) +
...
```{r}
A <- plotMatchPos(
  bm_pos, se = FALSE, abs = motif_params$analysis$abs, linewidth = 1/2,
  ...
  theme(legend.position = "none")
B <- plotMatchPos(
  bm_pos, type = "cdf", geom = "line", abs = motif_params$analysis$abs,
...
res_pos %>% 
  ...
  left_join(motif_list, by = "altname") %>% 
  to_list() %>% 
  getPwmMatches(fw_seq) %>% 
  plotMatchPos(
