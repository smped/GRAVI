...
In addition to enrichment of pathways, the consesus peaks were assessed for their relationship to the pre-defined gene-regions`r ifelse(has_features, " and the supplied external features.", ".")`.
The package `regioneReloaded` [Malinverni2023-ee] was used, which uses a permutation-based strategy to determine the expected relationship between a random region-set and the regions of interest.
The observed relationship is then assessed to the expected relationship to return normalised Z-scores and p-values as a measure of an above (or below) expected relationship.
The Z-score distributions are obtained by shifting the peaks using permutations, whilst holding the position of the regions-of-interest steady, and represent the relationship between observed overlaps and expected overlaps from permutations.

The primary question assessed in this manner is the relationship between the set of consensus peaks, and the pre-defined gene-regions`r ifelse(has_features, ", along with any provided external features.", ".")`.
By using a permutation-based strategy, regions of varying width, or covering varying proportions of the genome, are able to be analysed in an appropriate manner.
...
tbl <- slot(mlz, "multiLocalZscores")$resumeTab %>% 
  as_tibble() %>% 
  ...
  dplyr::select(region, n_overlaps, prop, norm_zscore, adj.p_value) %>% 
  ...
  reactable(
    columns = list(
      ...
      norm_zscore = colDef(
        "Z<sub>norm</sub>", format = colFormat(digits = 3), html = TRUE
      ),
      adj.p_value = colDef(
        glue("p<sub>{adj_method}</sub>"), html = TRUE,
        cell = \(value) sprint_pval(value)
      )
    )
  )
