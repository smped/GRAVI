...
```{r all-reps-upset, fig.cap = "*UpSet plot showing all samples including those which failed prior QC steps. Any potential sample mislabelling will show up clearly here as samples from each group should show a preference to overlap other samples within the same treatment group. However, this can still be dependent on the success of each individual IP*"}
samp_cols <- setNames(colours$treat[as.character(samples$treat)], samples$label)
lwr <- 0.2 + nrow(samples) / 60
individual_peaks %>%
  unlist() %>%
  select(sample) %>%
  reduceMC() %>%
  ...
  upset(
    ...
    mb.ratio = c(1 -lwr, lwr), 
    text.scale = 1.2, point.size = 2.5,
    nintersects = 35,
