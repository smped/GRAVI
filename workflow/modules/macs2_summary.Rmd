...
if (!is.null(config$external$features)) {
  ...
  feature_colours <- colours$features %>% setNames(str_sep_to_title(names(.)))
  has_features <- TRUE
}
...
macs2_logs %>%
  ggplot(
    aes(label, n_tags_treatment/1e6, fill = qc)
  ) +
  ...
  geom_hline(
    ...
    data = . %>%
      group_by(treat) %>%
      summarise(mn = mean(n_tags_treatment/1e6)),
    linetype = 2,
    col = "grey"
  ) +
...
suppressWarnings(
  macs2_logs %>%
    ...
    geom_label(
      ...
      nudge_y = max(macs2_logs$total_peaks)/30,
      inherit.aes = FALSE,
      show.legend = FALSE
    ) +
    facet_grid(~treat, scales = "free_x", space = "free_x") +
...
file.path(macs2_path, "cross_correlations.tsv") %>% 
  ...
  arrange(desc(treat), !!sym(rep_col)) %>% 
  ...
  facet_wrap(as.formula(paste("~ treat +", rep_col))) +
...
In the case of four or more replicates, the package `ComplexUpset` [@KrassowskiCompUp] was used to create UpSet plots [@Upset2014].
...
rep_colours <- hcl.colors(3, palette = "Zissou", rev = TRUE)
htmltools::tagList(
      ...
      function(i) {
        ...
        if (length(x) == 2) {
          ...
          args <- glue(
            ...
            "-c1 '{rep_colours[[1]]}' -c2 '{rep_colours[[3]]}' ",
        ...
        if (length(x) == 3) {
          ...
          args <- glue(
            ...
            "-c1 '{rep_colours[[1]]}' -c2 '{rep_colours[[2]]}' -c3 '{rep_colours[[3]]}' ",
            "-ht {knitr::opts_chunk$get('fig.height')} ", 
            "-w {knitr::opts_chunk$get('fig.width')} ", 
            "-o '{fig_out}'"
          )
        ...
        if (length(x) > 3) {
          ...
          p <- fromList(x) %>% 
            upset(
              ...
              order.by = "freq",
              set_size.show = TRUE,
              set_size.scale_max = max(colSums(.)) * 1.2, 
              text.scale = 1.2, 
              point.size = 3,
              sets.x.label = glue("Macs2 Peaks (FDR < {config$peaks$macs2$fdr})"),
              mainbar.y.label = "Peaks In Intersection"
            )
          print(p)
          dev.off()
        }
...
consensus_peaks %>% 
    ...
    plotSplitDonut(
        inner = "Feature", outer = "Region", min_p = 0.02, r_outer = 0.75,
        inner_glue = "{str_wrap(.data[[inner]], 12)}\n{percent(p, 0.1)}",
        outer_glue = "{str_wrap(.data[[outer]], 12)}\n{percent(p, 0.1)}",
        inner_palette = feature_colours,
...
htmltools::tagList(
  mclapply(
    ...
    function(x) {
      ...
      plotHFGC(
        grl_to_plot[[x]],
        features = feat_gr, featcol = feat_colours_hfgc, featsize = 1 + has_features,
      ...
      ## Define the caption
      gr <- join_overlap_inner(grl_to_plot[[x]], all_gr$gene)
      if (length(gr)) {
        d <- 0
      } else {
      ...
      gn <- collapseGenes(gr$gene_name, format = "")
      peak_desc <- ifelse(
        nm == "consensus",
      ...
      cp <- htmltools::tags$em(
        glue(
          ...
          "
          Y-axes for each coverage track are set to the maximum observed signal
