...
source(here::here("workflow/scripts/biovenn.R"))
...
For three or fewer replicates, Euler (Venn) Diagrams were generated using a modified version of the plotting function provided in the CRAN package `BioVenn` [@HulsenBiovenn2021].
...
```{r all-reps-upset, fig.cap = "*UpSet plot showing all samples including those which failed prior QC steps. Any potential sample mislabelling will show up clearly here as samples from each group should show a preference to overlap other samples within the same treatment group.*"}
samp_cols <- setNames(colours$treat[as.character(samples$treat)], samples$label)
individual_peaks %>%
  unlist() %>%
  select(sample) %>%
  reduceMC() %>%
  ...
  upset(
    ...
    sets.bar.color = samp_cols[colnames(.)],
    set_size.show = TRUE,
    set_size.scale_max = 1.2 * max(colSums(.))
  )
```
  ...
  names(replicates_by_treat) %>% 
    lapply(
      function(i) {
        ...
        png_path <- file.path(fig_path, glue("{i}_replicates.png"))
        if (length(x) == 1) {
          png(
            png_path, 
        ...
        if (length(x) %in% 2:3) {
          ...
          biovenn(
            x, 
            title = glue("{target}: All {i} Replicates")
          )
          dev.off()
        }
        ...
        cp <- htmltools::tags$em(
          glue(
            "
            Number of {target} peaks detected by macs2 callpeak in each {i} replicate 
            and then number of peaks shared between replicates.
            "
          )
        )
        
        ## Create html tags
        fig_link <- str_extract(png_path, "assets.+")
        htmltools::div(
          htmltools::div(
...
png_out <- file.path(fig_path, fig_name)
...
`r if(length(vd) < 3) glue("![*Number of {target} consensus peaks which overlap oracle peaks defined within each condition.*](assets/{target}/{target}_common_peaks.png)")`
