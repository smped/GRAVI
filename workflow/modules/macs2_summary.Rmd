...
As a brief overview of pathways and genes likely to be regulated by `r target`, the package `rGREAT` [@Gu2023-rp] was used to test for enrichment of pathways and genesets, as an R implementation of the GREAT methodology [@McLean2010-gw].
...
For this specific analysis, the background ranges were set to be the whole genome, excluding the ranges defined as gaps by UCSC.
...
`r if (n_great > 0) "#### Peak Distribution"`
...
top_matches <- motif_list %>% 
  ...
  to_list() %>% 
  getPwmMatches(
    fw_seq, abs = motif_params$analysis$abs, best_only = TRUE, mc.cores = threads
  )
top_matches %>% 
  plotMatchPos(
