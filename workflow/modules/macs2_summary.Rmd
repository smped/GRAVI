  ...
  names(replicates_by_treat) %>% 
    lapply(
      function(i) {
        ...
        ## Draw a circle for one replicate
        if (length(x) == 1) sets <- setNames(length(x[[1]]), names(x)[[1]])
        ## Euler diagrams for 2-3 replicates
        if (length(x) == 2) {
          sets <- c(
            length(setdiff(x[[1]], x[[2]])),
            length(setdiff(x[[2]], x[[2]])),
            length(intersect(x[[1]], x[[2]]))
          ) %>% 
            setNames(
              c(names(x), paste(names(x), collapse = "&"))
            )
        }
        if (length(x) == 3) {
          sets <- c(
            length(setdiff(x[[1]], unlist(x[-1]))),
            length(setdiff(x[[2]], unlist(x[-2]))),
            length(setdiff(x[[3]], unlist(x[-3]))),
            length(intersect(x[[1]], x[[2]])),
            length(intersect(x[[1]], x[[3]])),
            length(intersect(x[[2]], x[[3]])),
            length(intersect(x[[1]], x[[2]]) %>% intersect(x[[3]]))
          ) %>% 
            setNames(
              c(
                names(x), 
                paste(names(x)[1:2], collapse = "&"),
                paste(names(x)[-2], collapse = "&"),
                paste(names(x)[2:3], collapse = "&"),
        ...
        if (length(x) <= 3) {
          ...
          grid.draw(
            plot(
              ...
              fills = list(
                fill = rep(colours$treat[[i]], length(x)),
                alpha = 0.3
              ),
        ...
        if (length(x) > 3) {
          ...
          p <- fromList(x) %>% 
            upset(
              ...
              order.by = "freq",
              set_size.show = TRUE,
              set_size.scale_max = max(colSums(.)) * 1.15, 
              text.scale = 1.2, 
              point.size = 3
            )
          print(p)
        ...
        cp <- htmltools::tags$em(
          glue(
            "
            Number of peaks detected by macs2 callpeak in each {i} replicate 
