...
blacklist <- importPeaks(
  here::here(config$external$blacklist), type = "bed", seqinfo = sq
) %>% 
  unlist() %>% 
  unname()
## Greylist
greylist <- importPeaks(
  here::here("output", "annotations", glue("{ip}_greylist.bed")),
  type = "bed", seqinfo = sq
) %>% 
  unlist() %>% 
  unname()
...
individual_peaks <- file.path(
  ...
  importPeaks(seqinfo = sq, blacklist = exclude_ranges) %>%
...
treatment_peaks <- macs2_path %>% 
  file.path(target, glue("{target}_treatment_peaks.rds")) %>% 
  read_rds()
union_peaks <- macs2_path %>% 
  file.path(target, glue("{target}_union_peaks.bed")) %>% 
  importPeaks(
    type = "bed", blacklist = exclude_ranges, seqinfo = sq, setNames = FALSE
  ) %>% 
  unlist() %>% 
  mutate(
...
if (has_features) {
  union_peaks$Feature <- bestOverlap(
    union_peaks, external_features, var = "feature", missing = "no_feature"
  ) %>%
...
```{r treatment-peaks, echo=FALSE, eval=FALSE}
...
Treatment peaks which overlapped a peak in more than `r percent(min_prop)` of the *individual replicates passing QC* in each treatment group, were retained.
...
treat_levels %>% 
  lapply(
    \(i) tibble(
      ...
      retained = length(treatment_peaks[[i]]),
      `% retained` = percent(retained / detected_peaks, 0.1),
      median_width = median(width(treatment_peaks[[i]]))
    )
  ) %>% 
  bind_rows() %>% 
  ...
  pander(
    ...
    caption = glue(
      ...
      "{percent(min_prop)} of the retained samples for each", "treatment group, as described above.",
      .sep = " "
    )
  )
```
...
```{r plot-common-peaks, fig.cap = glue("*Number of {target} union peaks which overlap the treatment peaks defined within each condition.*")}
plotOverlaps(
  treatment_peaks, 
...
grl_to_plot$union <- union_peaks[union_peaks$score == max(union_peaks$score)] %>% 
  granges()
for (i in treat_levels) {
  grl_to_plot[[i]] <- treatment_peaks[[i]] %>% 
...
These are the most highly ranked Union Peak (by `score`) and the most highly ranked treatment peak for each of `r pander(treat_levels)` after incrementally stepping through and excluding any already selected peak.
...
The pathways tested were `r pander(unname(unlist(extra_params$enrichment$msigdb)))` as defined by MSigDB v`r packageVersion("msigdbr")`.
Pathways will be considered as enriched relative to background if returned an adjusted p-value (`r extra_params$enrichment$adj`) below `r extra_params$enrichment$alpha`, using the results from the *hypergeometric test*, instead of the *binomial test*.


```{r setup-great}
source(here::here("workflow", "scripts", "great_functions.R"))
...
great_cats <- map_great_cats(unlist(extra_params$enrichment$msigdb))
stopifnot(length(great_cats) > 0)
ext_tss <- gtf_gene %>% 
  select(gene_id) %>% 
  extendTSS(
    genome = map_great_refs(config$genome$build),
...
```{r run-great}
great_res <- multi_set_great(
  set_genome_info(
    union_peaks, 
    genome = map_great_refs(genome(union_peaks)[[1]])
  ), 
  gene_sets = great_cats, tss = ext_tss, 
  min_gene_set_size = extra_params$enrichment$min_size,
  max_gene_set_size = extra_params$enrichment$max_size,
  adj = adj_method, 
  min_hits = extra_params$enrichment$min_sig,
  great_cores = threads, verbose = FALSE
)
ext_tss_ol <- subsetByOverlaps(ext_tss, union_peaks)
great_df <- getEnrichmentTable(great_res) %>% 
  dplyr::select(-all_of(c("p_adjust", "p_value"))) %>% 
  as_tibble() %>% 
  dplyr::filter(p_adjust_hyper < enrich_alpha) %>% 
  mutate(
    gene_id  = slot(great_res, "gene_sets")[id],
    genes_with_hits = map_chr(
      gene_id,
      \(x) {
        ids <- subset(ext_tss_ol, gene_id %in% x)$gene_id
        paste(sort(id2gene[unique(ids)]), collapse = "; ")
      }
    )
  )
n_great <- nrow(great_df)
```
...
tbl <- great_df %>% 
  ...
  dplyr::select(id, fold_enrichment, p_adjust_hyper, everything()) %>% 
  reactable(
    ...
    columns = list(
      ...
      p_adjust_hyper = colDef(
        name = glue("P<sub>adj</sub> ({extra_params$enrichment$adj})"),
...
cp <- htmltools::em(
  glue(
    ...
    determine relative enrichment were taken as the entire genome.
    "
  )
)
if (n_great) {
