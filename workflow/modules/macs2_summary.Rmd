...
## Inspection Of Select Peaks {.tabset}

```{r grl-to-plot}
grl_to_plot <- vector("list", 3) %>% 
  ...
  lapply(
    function(x) {
      ...
      oracle_peaks[[x]] %>% 
        ...
        as_tibble() %>% 
        arrange(desc(score)) %>% 
        dplyr::slice(seq_along(treat_levels)) %>% 
        colToRanges("range", seqinfo = sq)
    }
  )
...
`r if ("consensus" %in% names(grl_to_plot)) "### Highest Ranked Consensus Peak"`

```{r cp-consensus, eval = "consensus" %in% names(grl_to_plot), echo = FALSE}
gr <- join_nearest(grl_to_plot$consensus, all_gr$gene, distance = TRUE)
d <- gr$distance
gn <- gr$gene_name
cp <- glue(
  "*The most highly ranked consensus peak by combined score across all treatments. ",
  ifelse(
  d == 0,
  paste('The peak directly overlapped', gn),
  paste0("The nearest gene was ", gn, ", which was ", round(d/1e3, 1), "kb away")
  ),
  ifelse(
...
```{r hfgc-consensus, eval = "consensus" %in% names(grl_to_plot), echo = "consensus" %in% names(grl_to_plot), fig.cap = cp}
plotHFGC(
  grl_to_plot$consensus,
  cytobands = bands_df,
...
`r if (treat_levels[[1]] %in% names(grl_to_plot)) paste0("### Highest Ranked Peak:", treat_levels[[1]])`

```{r cp-treat1, eval = treat_levels[[1]] %in% names(grl_to_plot), echo = FALSE}
gr <- join_nearest(grl_to_plot[[treat_levels[[1]]]], all_gr$gene, distance = TRUE)
d <- gr$distance
gn <- gr$gene_name
cp <- glue(
  "*The most highly ranked {treat_levels[[1]]}-unique oracle peak by score, ",
  "excluding the most highly-ranked consensus peak. ",
  ifelse(
  d == 0,
  paste('The peak directly overlapped', gn),
  paste0("The nearest gene was ", gn, ", which was ", round(d/1e3, 1), "kb away")
...
```{r hfgc-treat1, eval = treat_levels[[1]] %in% names(grl_to_plot), echo = treat_levels[[1]] %in% names(grl_to_plot), fig.cap = cp}
plotHFGC(
  grl_to_plot[[treat_levels[[1]]]],
  cytobands = bands_df,
  features = feat_gr, featcol = feature_colours,
  genes = hfgc_genes, genecol = gene_col,
  collapseTranscripts = FALSE,
...
`r if (treat_levels[[2]] %in% names(grl_to_plot)) paste0("### Highest Ranked Peak:", treat_levels[[2]])`

```{r cp-treat2, eval = treat_levels[[2]] %in% names(grl_to_plot), echo = FALSE}
gr <- join_nearest(grl_to_plot[[treat_levels[[2]]]], all_gr$gene, distance = TRUE)
d <- gr$distance
gn <- gr$gene_name
cp <- glue(
  "*The most highly ranked {treat_levels[[2]]}-unique oracle peak by score, ",
  "excluding the most highly-ranked consensus peak. ",
  ifelse(
  d == 0,
  paste('The peak directly overlapped', gn),
...
```{r hfgc-treat2, eval = treat_levels[[2]] %in% names(grl_to_plot), echo = treat_levels[[2]] %in% names(grl_to_plot), fig.cap = cp}
plotHFGC(
  grl_to_plot[[treat_levels[[2]]]],
  cytobands = bands_df,
  features = feat_gr, featcol = feature_colours,
  genes = hfgc_genes, genecol = gene_col,
  collapseTranscripts = FALSE,
