...
network_params <- here::here("config", "params.yml") %>% 
  read_yaml() %>% 
  pluck("networks")
motif_params <- config$motif_analysis$default
if (target %in% names(config$motif_analysis)) {
  for (i in names(config$motif_analysis[[target]])) {
    motif_params[[i]] <- config$motif_analysis[[target]][[i]]
  }
}
...
motif_list <- file.path(annotation_path, "motif_list.rds") %>% 
  ...
  to_df %>% 
  as_tibble()
motif_uri <- file.path(annotation_path, "motif_uri.rds") %>% 
  read_rds()
...
enrich_res <- peak_path %>% 
  ...
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_enrich <- sum(enrich_res$adj_p < motif_params$alpha & enrich_res$Z > 0)
```
