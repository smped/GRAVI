...
`r if(is.null(external_features), "", "### External Features And Gene-Centric Regions")`

```{r plot-feature-region-overlap, eval = !is.null(external_features), echo = !is.null(external_features), fig.height=8, fig.width=8, fig.cap = glue("*Proportions of {target} consensus peaks which overlap external features and gene-centric regions. Percentages in each panel sum to 100.*")}
suppressWarnings(
  gene_regions %>% 
    ...
    geom_xsidelabel(
      aes(region, p + 0.12*max(p), label = percent(p, 0.1)),
      data = . %>%
        group_by(region) %>%
        summarise(p = sum(p)),
      inherit.aes = FALSE
    ...
    geom_ysidelabel(
      aes(max(p)*0.15, feature, label = percent(p, 0.1)),
      data = . %>% 
        group_by(feature) %>% 
        summarise(p = sum(p)),
      inherit.aes = FALSE,
