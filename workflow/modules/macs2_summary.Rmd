...
macs2_fdr <- config$macs2$default$fdr
if (target %in% names(config$macs2)) macs2_fdr <- config$macs2[[target]]$fdr
outlier_thresh <- config$macs2_qc$default$outlier_threshold
if (target %in% names(config$macs2_qc)) 
  outlier_thresh <- config$macs2_qc[[target]]$outlier_threshold
...
macs2_logs %>%
  ...
  pander(
    ...
    caption = glue(
      ...
      "threshold of {percent(macs2_fdr)} during the peak calling ", 
      "process.",
      ifelse(
        any_fail,
        glue(
