...
```{r plot-common-peaks, fig.cap = glue("*Number of {target} consensus peaks which overlap the filtered peaks defined within each condition.*")}
plotOverlaps(
  filtered_peaks, 
...
great_res <- multi_set_great(
  set_genome_info(
    consensus_peaks, 
    genome = map_great_refs(genome(consensus_peaks)[[1]])
  ), 
