...
fdr_macs2 <- config$macs2$default$fdr
if (!is.null(config$macs2$default$fdr)) {
  fdr_macs2 <- config$macs2$default$fdr
}
thresh <- config$macs2_qc$default$outlier_threshold
if (!is.null(config$macs2_qc[[target]]$outlier_threshold)) {
  thresh <- config$macs2_qc[[target]]$outlier_threshold
}
...
macs2_logs %>%
  ...
  pander(
    ...
    caption = glue(
      ...
      "threshold of {percent(fdr_macs2)} during the peak calling ", 
      "process.",
      ifelse(
        any_fail,
        glue(
          ...
          "+/- {thresh}-fold the median number of ",
...
```{r plot-macs2-peaks-individual, fig.height=6, fig.cap = glue("*Peaks identified for each {target} sample. The number of peaks passing the inclusion criteria for `macs2 callpeak` (FDR < {fdr_macs2}) are provided. Any samples marked for exclusion are coloured as indicated in the figure legend.*")}
suppressWarnings(
  macs2_logs %>%
...
macs2_path %>%
  ...
  facet_grid(vars(treat), scales = "free", space = "free") +
  ...
  plotOverlaps(
    ...
    set_sizes = (
      upset_set_size() +
        ...
        ylab(glue("Macs2 Peaks (FDR < {fdr_macs2})")) +
        theme(
          panel.grid = element_blank(),
          axis.line = element_line(colour = "grey20"),
          panel.border = element_rect(colour = "grey20", fill =  NA)
