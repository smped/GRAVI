...
gs_list <- msigdb %>% 
  ...
  .[map_int(., length) <= max(enrich_params$size)] %>% 
  .[map_int(., length) >= min(enrich_params$size)] 
gs_url <- msigdb %>% 
...
great_res <- consensus_peaks %>% 
  ...
  great(
    ...
    min_gene_set_size = min(enrich_params$size), 
    cores = threads
  )
