...
```{r plot-common-peaks, results = 'hide', fig.height = ifelse(length(treat_levels) <= 3, 9, 6), fig.width = ifelse(length(treat_levels) <= 3, 6, 10), fig.cap="*Number of consensus peaks that overlap the oracle peaks defined in each condition.*"}
...
if (length(treat_levels) > 3) {
  ...
  file.create(file.path(fig_path, fig_name)) # An empty file to keep snakemake happy
}
```

`r ifelse(length(treat_levels) <= 3, glue("![*Number of consensus peaks which overlap oracle peaks defined within each condition.*](assets/{target}/{target}_common_peaks.png)"), "")`
...
consensus_peaks %>% 
  ...
  plotPie(
    fill = "detected", x = "region", scale_by = "width", min_p = 0.05,
...
consensus_peaks %>% 
  mutate(
    Feature = bestOverlap(
      ., external_features, var = "feature", missing = "no_feature"
    ) %>% 
