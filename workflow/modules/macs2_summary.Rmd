...
# Now break the peaks into detected / undetected & split the pie
      ...
      function(x) {
        list(
          tibble(
            ...
            all = propOverlap(consensus_peaks, x),
            detected = propOverlap(consensus_peaks, subset(x, detected))
          )
        )
      }
    ) %>% 
...
```{r plot-feature-overlap, eval = has_features, echo = has_features, fig.height=8, fig.width=8, fig.cap = glue("*Proportions of {target} consensus peaks which overlap external features.*")}
external_features %>% 
  splitAsList(.$feature) %>% 
  lapply(
    function(x) propOverlap(consensus_peaks, x)
  ) %>% 
  as_tibble() %>% 
  ...
  labs(fill = "External Feature") +
  theme_void()
```

`r ifelse(has_features, "### External Features And Gene-Centric Regions", "")`

```{r plot-feature-region-overlap, eval = has_features, echo = has_features, fig.cap = glue("*Proportions of {target} consensus peaks which overlap external features and gene-centric regions. Percentages in each panel sum to 100.*")}
suppressWarnings(
  gene_regions %>% 
    ...
    mutate(p = propOverlap(., consensus_peaks)) %>% 
    subset(p > 0) %>% 
    mutate(bp = p * width) %>% 
    mcols() %>% 
    as_tibble() %>% 
    ...
    ggplot(
      aes(region, feature, size = bp/1e6, alpha = bp/1e6, label = percent(p, 0.1))
    ) +
    geom_point(colour = "darkblue") +
    geom_label(size = 3) +
    geom_xsidecol(
      aes(region, p, fill = region),
      inherit.aes = FALSE,
      data = . %>% 
        group_by(region) %>% 
        summarise(p = sum(p))
    ...
    geom_xsidelabel(
      aes(region, p + 0.15*max(p), label = percent(p, 0.1)),
      data = . %>%
        group_by(region) %>%
        summarise(p = sum(p)),
      size = 3,
    ...
    geom_ysidelabel(
      aes(max(p)*0.2, feature, label = percent(p, 0.1)),
      data = . %>% 
        group_by(feature) %>% 
        summarise(p = sum(p)),
      inherit.aes = FALSE,
    ...
    scale_xsidey_continuous(expand = expansion(c(0, 0.15)), labels = NULL, breaks = NULL) +
    scale_ysidex_continuous(expand = expansion(c(0, 0.15)), labels = NULL, breaks = NULL) +
    scale_fill_manual(
      values =  colours$regions %>%
        setNames(regions[names(.)]) %>%
        c(
