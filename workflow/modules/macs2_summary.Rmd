  ...
  upset(
    ...
    base_annotations = list(
      `Peaks in Intersection` = intersection_size(
        bar_number_threshold = 1, text_colors = "black", text = list(size = 3)
      ) +
    ...
    set_sizes = (
      upset_set_size() +
        geom_text(
          aes(label = after_stat(count)), hjust = 1.1, stat = 'count', size = 3
        ) +
        scale_y_reverse(expand = expansion(c(0.2, 0))) +
...
```{r plot-common-upset, eval = length(vd) > 3, echo = length(vd) > 3, fig.height = 7, fig.cap = glue("*Number of {target} consensus peaks which overlap the oracle peaks defined within each condition.*")}
