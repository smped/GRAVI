...
```{r barcode-gsea-dir-sig, eval = length(p) > 0, echo = length(p) > 0, fig.height= 3 * ceiling(length(p) / 3), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __directional__ changes in gene expression.*")}
...
```{r barcode-gsea-nondir-sig, eval = length(p) > 0, echo = length(p) > 0, fig.height= 3 * ceiling(length(p) / 3), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __non-directional__ changes in gene expression.*")}
plot_grid(plotlist = p, nrow = ceiling(length(p) / 3))
```
