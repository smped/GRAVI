...
cmn_res <- goseq_group_res %>% 
  ...
  lapply(
    ...
    fdr = p.adjust(pval, adj_method)
  ) %>% 
  lapply(dplyr::filter, fdr < enrich_alpha) %>% 
  lapply(
    mutate,
    status = case_when(
      adj_p < enrich_alpha & padj < enrich_alpha ~ "Both",
      !adj_p < enrich_alpha & padj < enrich_alpha ~ "RNA Only",
      adj_p < enrich_alpha & !padj < enrich_alpha ~ "ChIP Only",
      !adj_p < enrich_alpha & !padj < enrich_alpha ~ "Neither"
    ),
  ...
  lapply(
    select,
    group, gs_name, NES, status, adj_p_chip = adj_p, adj_p_rna = padj, 
    adj_p = fdr, starts_with("num"), leadingEdge, genes
  ) %>% 
  lapply(dplyr::filter, str_detect(genes, ";")) %>% 
  .[vapply(., nrow, integer(1)) > 0] 
tg_cmn <- lapply(cmn_res, make_tbl_graph, gs = gs_by_gsid) %>% 
  .[vapply(., length, integer(1)) >= min_network_size]
any_sig <- length(cmn_res) > 0
plot_net <- length(tg_cmn) > 0
txt <- ifelse(any_sig, "### Results Tables {.tabset}", "No shared enrichment was found")
```
  ...
  names(cmn_res) %>% 
    lapply(
      function(x) {
        df <- cmn_res[[x]] %>% 
          dplyr::select(
            gs_name, adj_p, NES, status, starts_with("adj_p_"), numDEInCat, genes
          )
        tbl <- df %>% 
          reactable(
    ...
    mclapply(
      function(x) {
        ...
        p <- tg_cmn[[x]] %>%
          ...
          geom_node_point(
            aes(fill = -log10(adj_p), size = numDEInCat),
            shape = 21
          ) +
          geom_node_text(
