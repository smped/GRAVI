...
```{r barcode-gsea-dir-sig, dev = 'png', eval = length(p) > 0, echo = length(p) > 0, fig.height= 3 * ceiling(length(p) / 3), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __directional__ changes in gene expression.*")}
...
`r if (nrow(gsea_nondir_sig) == 0) glue("No association was found between changed binding across {comps[[1]]} and {comps[[2]]} when combined with *overall* significance in differentially expressed genes.")`
...
```{r barcode-gsea-nondir-sig, dev = 'png' eval = length(p) > 0, echo = length(p) > 0, fig.height= 3 * ceiling(length(p) / 3), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __non-directional__ changes in gene expression.*")}
plot_grid(plotlist = p, nrow = ceiling(length(p) / 3))
```
...
cmn_res <- goseq_group_res %>% 
  ...
  lapply(
    ...
    status = case_when(
      ...
      factor(levels = c("Both", "ChIP Only", "RNA Only", "Neither")),
