...
```{r plot-volcano, fig.cap = glue("*Volcano plots for differential expression separated by target binding patterns. Many genes will have multiple target binding regions mapped to them, and may appear in multiple panels. The two ost highly ranked genes for differential expression are labelled in eachpanel. All DE genes are listed with full binding patterns in the table below.*")}
diff_windows %>% 
