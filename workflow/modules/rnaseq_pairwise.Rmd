...
```{r plot-volcano, fig.cap = glue("*Volcano plots for differential expression separated by target signal patterns. Some genes will have multiple target signal regions mapped to them, and may appear in multiple panels. The two most highly ranked genes for differential expression are labelled in each panel. All DE genes are listed with full patterns in the table below.*")}
ids_by_group %>% 
...
`r if (any_de) "## DE Genes And Pairwise Signal"`
...
htmltools::tags$caption(
  htmltools::em(
    glue(
    "
    All differentially expressed genes where signal was detected in both 
    comparisons.
    "
    )
  )
...
GSEA analysis was performed taking the set of genes mapped to each signal pattern.
...
This second analysis allows for detecting regulatory changes due to altered signal, but which are spread between both up- and down-regulation, as may be induced by the presence of other co-factors not under direct investigation here.
...
`r if (length(p) == 0) glue("No association was found between joint signal patterns and *directional* significance in differentially expressed genes.")`
...
txt <- ifelse(
  length(p) > 0,
  glue(""),
  glue("No association was found between changed signal across {comps[[1]]} and {comps[[2]]} when combined with *overall* significance in differentially expressed genes.")
)
```
...
Directional GSEA was performed on the set of RNA-Seq results and given that enrichment for differential ChIP signal target is an orthogonal measure, enrichment for a set of potential pathways for each pairwise set of regions was performed.
The p-values from signal enrichment and GSEA were combined using Wilkinson's method for meta-analysis, taking the maximum p-value and adjusting the resulting p-value using the same strategy as earlier.
Any pathways are annotated as being significant in *both, either or neither* dataset when being analysed individually.

Distances between nodes (gene-sets) for any network plots were estimated by using the similarity of genes within the leading edge which were also mapped to signal regions showing the respective pattern.
    ...
    lapply(
      function(x) {
        ...
        cp <- glue(
          ...
          from RNA-Seq and enrichment anaylsis from sites corresponding 
