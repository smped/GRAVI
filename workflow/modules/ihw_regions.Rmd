...
ihw_all <- gene_regions %>% 
  .[names(.) != "tss"] %>% 
  lapply(
    function(x) overlapsProp(merged_results, x)
  ) %>% 
  as_tibble() %>% 
  mutate(
  ...
  arrange(id) %>% 
  .[["region"]]
merged_results$ihw_covariate <- fct_lump_min(ihw_all, min = 1e3)
ihw_proceed <- min(table(merged_results$ihw_covariate)) > 1e3
fdr_column <- ifelse(ihw_proceed, "fdr_ihw", "fdr")
...
Independent Hypothesis Weighting (IHW) was then used to partition the raw p-values by genomic regions defined previously, and estimate weights to apply to each p-value as a divisor.
...
tibble(ihw = ihw_all) %>% 
  ...
  geom_text(
    aes(y = n + 0.025*max(n), label = comma(n)),
    data = . %>%
