...
localz_df <- localz_regions %>%
  ...
  mutate(
    ...
    region = all_regions_labs[region],
...
shifted_z <- localz_regions %>% 
  ...
  mutate(
    ...
    Z = Z / sqrt(n),
    region = all_regions_labs[region] %>% factor(levels = all_regions_labs)
  ) 
...
.makeCaption <- function(.gr, target) {
  ...
  feat <- c()
  add_feat <- has_features & .gr$feature != 'no_feature'
  if (add_feat) 
    feat <- case_when(
    ...
    glue(
      ...
      ifelse(
        ...
        glue(
          ...
          a separate block beneath the gene-centric regions. "
        ),
        glue("")
      )
    ),
  ...
  mclapply(
    ...
    \(i) {
      ...
      plotHFGC(
        ...
        annotation = annot, annotcol = c("NFR" = "black"),
        cytobands = bands_df, 
        collapseTranscripts = "auto", maxTrans = 20,
        ylim = y_lim, zoom = 10, max = 1e4, title.width = 2
      )
      dev.off()
      ## Create html tags
