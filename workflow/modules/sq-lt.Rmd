...
The `quantro` test was first applied [@HicksQuantro2015] to determine if treatment-specific signal distributions were found in the data.
...
```{r plot-rle, dev = 'png', fig.height=6, fig.cap = glue("*RLE plot showing logCPM values. RLE values were calculated within each treatment group to account for the potentially different signal dynamics of {target}.*")}
filtered_counts %>% 
  plotAssayRle(
...
```{r plot-rle-normalised, dev = 'png', fig.height=6, fig.cap = glue("*RLE plot showing normalised logCPM. RLE values were calculated within each treatment group to account for the potentially different signal dynamics of {target}.*")}
filtered_counts %>% 
  plotAssayRle(
...
a <- plotAssayPCA(
  filtered_counts, "logCPM", colour = "treat", label = "label", 
  show_points = FALSE
) +
  scale_colour_manual(values = treat_colours) +
  guides(colour = "none") +
...
b <- plotAssayPCA(
  filtered_counts, "qsmooth", colour = "treat", label = "label",
  show_points = FALSE
) +
  scale_colour_manual(values = treat_colours) +
  guides(colour = "none") +
...
A simple linear model was fitted taking `r treat_levels[[1]]` as the baseline and estimating the effects of `r treat_levels[[2]]` on `r target` signal within each sliding window, incorporating a trended prior-variance.
`r txt`
