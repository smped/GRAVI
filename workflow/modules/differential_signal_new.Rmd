...
This process has produced counts for `r comma(nrow(counts))`.
...
if ("logCPM" %in% assayNames(counts)) {
  lcpm <- assay(counts, "logCPM")
} else {
  lcpm <- cpm(counts, log = TRUE)
}
qtest <- quantro(lcpm, counts$treat, B = 1e3)
quantro_p <- c(
...
if (!is.null(pair_col) & method == "lt") {
  ...
  ## lmFit, although when method is qflf they will be passed to glmQLFit.
  ...
  pair_cors <- duplicateCorrelation(
    object = assay(counts, assay_name)[ind, ],
    design = X,
    block = block
  )$consensus.correlation # This will be ignore in ... if block = NULL
  txt <- glue(
    "Data were nested within {{pair_col}} as a potential source of correlation. ",
...
fit <- fitAssayDiff(
  ...
  method = method, norm = norm, fc = fc, block = block, correlation = pair_cors
) 
if (win_type == "sliding") {
  results <- mergeByHMP(fit) %>% addDiffStatus(alpha = fdr_alpha)
  fdr_col <- "hmp_fdr"
} else {
  results <- rowRanges(fit) %>% addDiffStatus(alpha = fdr_alpha)
}
## At this point, extraChIPs needs to be updated to better manage column names
## when taking mergeByHMP. The
```
