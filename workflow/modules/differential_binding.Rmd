...
df <- merged_results %>% 
  select(status, region, feature) %>% 
  as_tibble() %>% 
  group_by(status, region, feature) %>% 
  summarise(n = dplyr::n(), .groups = "drop") %>% 
  complete(status, region, feature, fill = list(n = 0)) %>%
  pivot_wider(
    names_from = "status", values_from = n
  ) %>% 
  mutate(
    x = as.integer(region),
...
ggplot() + 
  geom_scatterpie(
    aes(x, y, r = 2*r), 
    data=df,
    cols=c("Up", "Down", "Unchanged")
  ) +
  geom_label(
    aes(x, y, label = comma(N, 1)),
    data = df %>% 
      dplyr::filter(r > 0.025),
    alpha = 0.5
  ) +
  coord_equal() +
  scale_x_continuous(
    breaks = seq_along(levels(df$region)),
    labels = levels(df$region) %>% 
      str_replace(" \\(", "\n(")
  ) +
  scale_y_continuous(
  ...
  labs(
    x = "Region", y = "External Feature", fill = "Status"
  )
```
...
line_col <- samples %>% 
  mutate(col = treat_cols[as.character(treat)]) %>% 
  split(.$label) %>%
  lapply(pull, "col") %>%
  lapply(setNames, c())
...
makeCaption <- function(.gr) {
  ...
  cp <- c(
    glue(
      ...
      ifelse(
        ...
        glue(
          ...
          "below the gene-centric regions. "
        ),
        glue("")
      ),
    ),
    ...
    "For each sample, the y-axis limits represent the values in the window with the highest signal.*"
  )
  paste(cp, collapse = " ")
}
```
