...
cp <- htmltools::em(
  glue(
    "The {show_n} most highly-ranked windows by FDR. ",
...
If `r min_network_size` or more gene-sets were considered to be enriched, a network plot will be produced for that specific analysis.
The distances between gene-sets were calculated using the overlap coefficient (OC), such that $d = 1 - OC$.
...
Gene-set (i.e. node) pairs with a distance > 0.9 will not have edges drawn between them and edge width also corresponds to the distance between nodes with closely related nodes having thicker edges.
All network plots were generated using the Fruchterman-Reingold (`fr`) layout algorithm.
...
tg_mapped <- make_tbl_graph(goseq_mapped, gs = gs_by_gsid)
...
```{r plot-network-mapped, eval = plot_network_mapped, echo = plot_network_mapped, fig.cap=glue("*Network plot showing all gene-sets enriched amongst the overall set of sites with a binding site for {target}. Distances were calculated using 1 - the overlap-coefficient with node-pair distances > 0.9 excluded from the final set of edges. Edge width corresponds to the number of shared genes between the two gene-sets.*")}
tg_mapped %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
    aes(fill = -log10(pval), size = 100 * prop),
    shape = 21
  ) +
  geom_node_text(
    aes(label = label),
    ...
    data = . %>%
      mutate(label = str_replace_all(label, "_", " ") %>% str_wrap(width = 15)),
    repel = TRUE,
    size = 3
  ) +
  ...
  scale_size_continuous(range = c(1, 10), breaks = seq(0, 100, by = 10)) +
  scale_edge_width(range = c(1, 6), trans = "reciprocal") +
  scale_edge_alpha(range = c(0.1, 0.4), trans= "reciprocal") +
...
tg_diff <- make_tbl_graph(goseq_diff, gs = gs_by_gsid)
...
```{r plot-network-diff, eval = plot_network_diff, echo = plot_network_diff, fig.cap=glue("*Network plot showing all gene-sets enriched amongst those differential signal for {target}. Distances were calculated using 1 - the overlap-coefficient with node-pair distances > 0.9 excluded from the final set of edges. Edge width corresponds to the number of shared genes between the two gene-sets.*")}
tg_diff %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
...
tg_up <- make_tbl_graph(goseq_up, gs = gs_by_gsid)
...
```{r plot-network-up, eval = plot_network_up, echo = plot_network_up, fig.cap=glue("*Network plot showing all gene-sets enriched amongst those showing increased signal for {target}. Distances were calculated using 1 - the overlap-coefficient with node-pair distances > 0.9 excluded from the final set of edges. Edge width corresponds to the number of shared genes between the two gene-sets.*")}
tg_up %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
...
tg_down <- make_tbl_graph(goseq_down, gs = gs_by_gsid)
...
```{r plot-network-down, eval = plot_network_down, echo = plot_network_down, fig.cap=glue("*Network plot showing all gene-sets enriched amongst those showing decreased signal for {target}. Distances were calculated using 1 - the overlap-coefficient with node-pair distances > 0.9 excluded from the final set of edges. Edge width corresponds to the number of shared genes between the two gene-sets.*")}
tg_down %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
