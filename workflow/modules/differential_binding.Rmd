...
plot_network_mapped <- length(tg_mapped) > 2
```
...
tg_mapped %>% 
  ggraph(layout = "stress", weights = 1/d^2) +
...
plot_network_diff <- length(tg_diff) > 2
```
...
plot_network_up <- length(tg_up) > 2
```
...
plot_network_down <- length(tg_down) > 2
```
