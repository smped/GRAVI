...
A dual filtering strategy was used based on retaining `r percent(config$comparisons$filter_q, 0.1)` of windows overlapping the consensus peaks identified by `macs2 callpeak` on merged samples.
...
```{r boxplot-regions, fig.height = 7, fig.cap = "*Distributions of signal (logCPM) and changed signal (logFC) for each genomic region, separated by whether the merged window overlapped a consensus peak defined by `macs2 callpeak`.*"}
df <- gene_regions %>% 
  .[names(.) != "tss"] %>% 
  ...
  cbind(
    merged_results %>% 
      select(
        all_of(
          c("AveExpr", "logFC", "P.Value", fdr_column, "status", "overlaps_ref")
        )
      ) %>% 
      mutate(
        range = as.character(.), id = seq_along(range)
      ) %>% 
      mcols()
  ) %>% 
  as_tibble() %>% 
  ...
  mutate(
    region = regions[region] %>% 
      ...
      fct_relabel(str_replace_all, pattern = " ", replacement = "\n")
  ) 
df %>% 
  ...
  ggplot(aes(region, value, fill = overlaps_ref)) +
  geom_boxplot() +
  facet_grid(
...
### Windows By Region and Overlap

```{r plot-ma-by-region, fig.cap = "*Binding intensity (logCPM) against changed signal (logFC) for all merged windows, separated by genomic region and consensus peak overlap. Significance and direction are indicated by colour*"}
df %>% 
  mutate(
    overlaps_ref = unlist(facet_labeller(overlaps_ref))
  ) %>% 
  ...
  facet_grid(region ~ overlaps_ref) +
  scale_colour_manual(
    values = colours$direction %>% 
      .[names(.) != "undetected"] %>% 
      setNames(str_to_title(names(.)))
  ) +
  labs(x = "logCPM", colour = "Status")
```

<!-- Should add optional plots by feature -->
