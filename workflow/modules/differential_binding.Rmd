...
```{r plot-bfl-summary, echo = FALSE, eval = FALSE, fig.height=6, fig.cap = glue("*Summary of A) Read Length and B) MAPQ scores for all samples along with the input. Values were obtained by sampling the first {comma(ys)} alignments from each bam file.*")}
a <- bfl_summary$qwidth %>%
	ggplot(aes(label, value, fill = treat)) +
	geom_boxplot(alpha = 0.8) +
	scale_fill_manual(values = treat_cols) +
	labs(
		x = "Sample", y = "Read Length", fill = "Treatment"
	)
b <- bfl_summary$mapq %>%
	ggplot(aes(label, value, fill = treat)) +
	geom_boxplot(alpha = 0.8) +
	scale_fill_manual(values = treat_cols) +
	labs(
		x = "Sample", y = "MAPQ", fill = "Treatment"
	)
plot_grid(
...
merged_results <- merged_results %>% 
  mapByFeature(
    genes = genes_gr, 
    prom = subset(feat_gr, feature == "Promoter"),
