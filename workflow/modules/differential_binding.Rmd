...
```{r make-greylist}
## This should really be run as a stand-alone script for each input file with
## a bed file exported once for each input file. These can then be loaded for 
## each relevant comparison. The approach would be to define a set in the 
## Snakefile using `unique(read_tsv(config$samples$file)$input)` and obtain the 
## greylist for each using a standalone script
ip <- file.path(
  bam_path, "Input", glue("{unique(samples$input)}.bam")
)
set.seed(100)
greylist <- ip %>% 
  bplapply(
...
```{r plot-rle-normalised, dev = 'png', fig.height=6, fig.cap = glue("*RLE plot showing logCPM values after Smooth Quantile normalisation. RLE values were calculated within each treatment group to account for the potentially different binding dynamics of {target}.*")}
filtered_counts %>% 
  plotAssayRle(
...
```{r plot-merged-size, dev='png', fig.height=7, fig.cap = glue("*Comparison of {target} signal intensity (logCPM) and merged window size (kb) for windows which overlap a macs2 consensus peak and those which don't. A mall amount of jitter has been added to the y-axis in the lower-left panel.*")}
merged_results %>% 
  mutate(kb = width / 1e3) %>% 
  as_tibble() %>% 
  mutate(
...
df <- merged_results %>% 
  ...
  mutate(
    Total = length(merged_results),
    `% Changed` = 1 - n_unch / Total
  ) %>% 
...
tbl <- df %>% 
  reactable(
    columns = list(
      Total = tbl_cols$Total[c("cell")],
      Up = tbl_cols$Up[c("cell", "style")],
      Down = tbl_cols$Down[c("cell", "style")],
      Unchanged = tbl_cols$Unchanged[c("cell", "style")],
      Ambiguous = tbl_cols$Ambiguous[c("cell", "style")],
      `% Changed` = tbl_cols$`% Changed`[c("cell", "style")]
    )[colnames(df)] %>% 
      lapply(set_attr, which = "class", value = "colDef")
  )
div(class = "table",
...
if (sum(pwf_diff$DEgenes) > min_genes & sum(pwf_diff$DEgenes) < max_genes) {
  goseq_diff <- goseq(
    pwf_diff, config$genome$build, gene2cat = gs_by_geneid, method = "Hypergeometric"
  ) %>% 
    dplyr::rename(gs_name = category, pval = over_represented_pvalue) %>% 
    dplyr::select(-starts_with("under")) %>% 
    as_tibble() %>% 
...
if (sum(pwf_up$DEgenes) > min_genes & sum(pwf_up$DEgenes) < max_genes) {
  goseq_up <- goseq(
    pwf_up, config$genome$build, gene2cat = gs_by_geneid, method = "Hypergeometric"
  ) %>% 
    dplyr::rename(gs_name = category, pval = over_represented_pvalue) %>% 
    dplyr::select(-starts_with("under")) %>% 
    as_tibble() %>% 
