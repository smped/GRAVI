...
msigdb <- msigdbr(species = "Homo sapiens") %>% 
  dplyr::filter(
    gs_cat %in% c("H") |
      gs_subcat %in% c("CP:KEGG", "CP:REACTOME", "CP:WIKIPATHWAYS", "TFT:GTRD"),
...
msigdb %>% 
  ...
  mutate(
    Description = case_when(
      ...
      Category == "C3" ~ str_replace_all(`Sub-Category`, "TFT:(.+)", "Transcription Factor Target Prediction Gene-Sets: \\1")
    ),
    Category = factor(Category, levels = c("H", "C2", "C3"))
  ) %>% 
  arrange(Category) %>% 
  pander(
    justify = "llrl", 
...
tbl <- goseq_mapped %>%
  ...
  dplyr::select(
    ...
    Mapped = numDEInCat, `Total Gene Set Size` = numInCat,
    p = pval, FDR = fdr, 
  ...
  reactable(
    ...
    columns = list(
      ...
      Mapped = colDef(maxWidth = 80),
      "Total Gene Set Size" = colDef(
        maxWidth = 100,
        cell = function(value) comma(value, 1)
      ),
      p = colDef(
...
tbl <- goseq_diff %>% 
  ...
  dplyr::select(
    `Gene Set` = gs_name, Description = gs_description,
    `Mapped To DB Windows` = numDEInCat, `Total Mapped` = numInCat,
    p = pval, FDR = fdr, 
  ...
  reactable(
    ...
    columns = list(
      "Gene Set" = colDef(
        minWidth = 200,
        cell = function(value) htmltools::tags$a(
          href = value$gs_url, target = "_blank", value$gs_name
        ),
        html = TRUE
      ),
      Description = colDef(
      ...
      "Mapped To DB Windows" = colDef(maxWidth = 100),
      "Total Mapped" = colDef(
        maxWidth = 100,
        cell = function(value) comma(value, 1)
      ),
...
tbl <- goseq_up %>% 
  ...
  dplyr::select(
    `Gene Set` = gs_name, Description = gs_description,
    `Mapped to Gained Windows` = numDEInCat, `Total Mapped` = numInCat,
    p = pval, FDR = fdr, 
  ...
  reactable(
    ...
    columns = list(
      ...
      "Mapped to Gained Windows" = colDef(maxWidth = 100),
      "Total Mapped" = colDef(
        maxWidth = 100,
        cell = function(value) comma(value, 1)
      ),
...
tbl <- goseq_down %>% 
  ...
  dplyr::select(
    `Gene Set` = gs_name, Description = gs_description,
    `Mapped to Decreased Windows` = numDEInCat, `Total Mapped` = numInCat,
    p = pval, FDR = fdr, 
  ...
  reactable(
    ...
    columns = list(
      ...
      "Mapped to Decreased Windows" = colDef(maxWidth = 100),
      "Total Mapped" = colDef(
        maxWidth = 100,
        cell = function(value) comma(value, 1)
      ),
