...
`r ifelse(sum(goseq_up$fdr < fdr_alpha) == 0, "No enrichment was found amongst genes mapped to site with increased binding", "")`
...
`r ifelse(sum(goseq_down$fdr < fdr_alpha) == 0, "No enrichment was found amongst genes mapped to site with decreased binding", "")`

```{r tbl-goseq-up, eval = sum(goseq_down$fdr < fdr_alpha) > 0, echo = sum(goseq_down$fdr < fdr_alpha) > 0}
cp <- htmltools::em(
  glue(
