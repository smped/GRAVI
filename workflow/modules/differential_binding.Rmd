...
all_out <- list(
  ...
  de_genes = file.path(
    out_path, glue("{target}_{treat_levels[1]}_{treat_levels[2]}_DE_genes.csv")
  ),
  renv = here::here(
    "output/envs",
    glue(
...
For subsequent classification when combining ChIP targets, an additional p-value testing $\mu = 0$ was added, however this is not used for detection of sites showing evidence of altered `r target` binding.
...
if (length(sig_ranges$Up) > 0) {
  merged_results %>% 
  subsetByOverlaps(sig_ranges$Up) %>% 
  granges() %>% 
...
if (length(sig_ranges$Down) > 0) {
  merged_results %>% 
  subsetByOverlaps(sig_ranges$Down) %>% 
  granges() %>% 
