...
peak_path <- here::here("output", "peak_analysis")
samples <- file.path(macs2_path, targets, "{targets}_qc_samples.tsv") %>% 
  glue() %>% 
  lapply(read_tsv) %>% 
  bind_rows() %>% 
...
shared_peaks <- file.path(peak_path, "shared", "shared_consensus_peaks.bed.gz") %>% 
  importPeaks(seqinfo = sq, type = "bed") %>% 
  setNames(str_remove(names(.), "_consensus.+")) %>% 
  unlist() %>% 
  unname() %>% 
...
localz_regions <- here::here(
  peak_path, targets, glue("{targets}_regions_localz.rds")
) %>% 
  c(here::here(peak_path, "shared", "shared_regions_localz.rds")) %>% 
...
```{r load-localz-peaks}
mlz_list <- here::here("output", "macs2", "shared", "all_consensus_localz.rds") %>% 
  read_rds
dist_df <- mlz_list %>% 
  lapply(
    function(x) {
...
```{r plot-shiftedz, fig.cap = "Distributions of shifted Z-scores for each target relative to other targets. Narrow peaks indicate close proximity of overlaps, whilst dips around zero indicate very close by not overlapping signal, as may be observed with histone marks and associated transcription factors."}
dist_df %>% 
  arrange(desc(Z)) %>%
  mutate(
...
motif_list <- file.path(annotation_path, "motif_list.rds") %>% 
  ...
  to_df %>% 
  as_tibble()
pos_res <- here::here(peak_path, "shared", "shared_motif_position.tsv.gz") %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_pos <- sum(pos_res$adj_p < motif_params$analysis$alpha)
enrich_res <-  here::here(peak_path, "shared", "shared_motif_enrichment.tsv.gz") %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_enrich <- sum(enrich_res$adj_p < motif_params$analysis$alpha)
matches <- file.path(peak_path, "shared", "shared_matches.rds") %>% 
  read_rds()
```
