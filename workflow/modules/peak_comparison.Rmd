...
```{r load-regioner}
localz_regions <- here::here(macs2_path, targets, glue("{targets}_regions_localz.rds")) %>% 
  c(here::here(macs2_path, "shared", "shared_regions_localz.rds")) %>% 
...
motif_list <- file.path(annotation_path, "motif_list.rds") %>% 
  ...
  to_df %>% 
  as_tibble()
pos_res <- here::here(macs2_path, "shared", "shared_motif_position.tsv.gz") %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_pos <- sum(pos_res$adj_p < motif_params$analysis$alpha)
enrich_res <-  here::here(macs2_path, "shared", "shared_motif_enrichment.tsv.gz") %>% 
  read_tsv() %>% 
  dplyr::select(-fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_adj))
n_enrich <- sum(enrich_res$adj_p < motif_params$analysis$alpha)
matches <- file.path(macs2_path, "shared", "shared_matches.rds") %>% 
  read_rds()
```
