...
target_colours <- RColorBrewer::brewer.pal(n_targets + 1, "Set1") %>% 
...
```{r plot-overlaps, fig.height=6, fig.cap = glue("*Taking the combined overlap between any targets as the set of regions, the number of regions with each target overlapping these regions is shown below*")}
plotType <- ifelse(length(consensus_peaks) > 2, "upset", "venn")
plotOverlaps(
...
```{r plot-top-great-genomic, fig.height=6, fig.cap = glue("*The distribution of shared regions around the TSS for the most highly ranked gene-set ({top_id})*")}
...
tbl <- localz_df %>% 
  ...
  reactable(
    columns = list(
      target = colDef("Target", filterable = TRUE),
      region = colDef("Region", minWidth = 120, filterable = TRUE),
      target_peaks = colDef("N Peaks", format = comma_col, maxWidth = 80),
      mean_perm_test = colDef("Expected", format = comma_col),
      n_overlaps = colDef("Observed", format = comma_col, maxWidth = 85),
      prop = colDef("% Peaks", format = percent_col, maxWidth = 80),
      Z = colDef(
