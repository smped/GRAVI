...
localz_regions <- here::here(
  peak_path, "{targets}", "{targets}_regions_localz.rds"
) %>% 
  glue() %>%
...
```{r load-localz-peaks}
mlz_list <- here::here(peak_path, "shared", "shared_targets_localz.rds") %>% 
  read_rds
dist_df <- mlz_list %>% 
  lapply(
    function(x) {
...
fw_peaks <- resize(
  shared_peaks, width = motif_params$analysis$peak_width, fix = "center"
)
