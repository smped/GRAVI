...
Independent Hypothesis Weighting (IHW) [@IgnatiadisIHW2016] was then used to partition the raw p-values for `r target` differential signal, by features provided in `r basename(config$external$features)`, and estimate weights to apply to each p-value as a divisor.
