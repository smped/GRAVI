...
```{r plot-feature-overlap, eval = has_features, echo = has_features, fig.height = 0.8 * fw, fig.width = 0.8 * fw, fig.cap = glue("*The total number of shared peaks which overlap external features provided in {basename(config$external$features)}. If peaks map to multiple features, they are assigned to the one with the largest overlap.*")}
shared_peaks %>% 
  plotPie(
...
```{r plot-feature-region-overlap, eval = has_features, echo = has_features, fig.cap = glue("*The total number of shared peaks overlapping external features and gene-centric regions. If a peak overlaps multiple features or regions, it is assigned to be the one with the largest overlap. Any peaks which don't overlap a feature have been excluded.*")}
shared_peaks %>% 
  plotSplitDonut(
...
htmltools::tagList(
  lapply(
    ...
    function(x) {
      ...
      ## Define the caption
      gn <- unlist(grl_to_plot[[x]]$gene_name)
      reg <- grl_to_plot[[x]]$region
