...
In addition, as set of *shared peaks* was defined as the genomic regions with detected signal in *m = max(n - 1, 2)* targets, where *n* represents the total number of targets
...
```{r tbl-rna-by-region, echo = has_rna}
if (has_rna) {
  tagList(
    names(rna) %>% 
      lapply(
...
all_out <- list(
  enrich_genomic_bg = here::here(
    "output", "results", "shared", "shared_enrichment_results_genomic_bg.tsv.gz"
  ),
  enrich_targets_bg = here::here(
    "output", "results", "shared", "shared_enrichment_results_targets_bg.tsv.gz"
  ),
