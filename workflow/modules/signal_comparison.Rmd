...
```{r plot-peaks-by-feature, echo = has_features, fig.height = ifelse(length(levels(p$data$feature)) < 3, fh / 2, fh), fig.cap = glue("*Shared peaks which overlap each provided feature, coloured by gene-centric regions defined previously. Features may overlap with each other, and as such, peaks may overlap multiple feature types and be included within multiple categories. Total overlapping peaks for each feature type are shown in bold, wth percentages being relative to the total number of shared peaks ({comma(length(shared_peaks))}). Individual segments are labelled if representing >= 3% of the total number of peaks. Features with fewer than 1% of peaks aligning to them are collapsed into the category 'Other'*")}
if (has_features) {
  p <- shared_peaks %>% 
    as_tibble() %>% 
    ...
    geom_label(
      ...
      data = . %>% 
        dplyr::filter(n > 0.0295 * length(shared_peaks)) %>% 
        mutate(
  ...
  lapply(
    ...
    function(i) {
      ...
      plotHFGC(
        ...
        featsize = length(external_features), featstack = "pack",
      ...
      cp <- htmltools::tags$em(
        glue(
          ...
          collapseGenes(gn), ", and is associated with {reg_pre} {reg}. ",
...
shifted_z %>% 
  ...
  scale_y_discrete(expand = rep_len(0, 4), name = NULL, labels = label_wrap(20)) +
