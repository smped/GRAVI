...
donuts <- rna %>% 
  ...
  # lapply(
  #   dplyr::filter, !grepl("detected", status.x) & !grepl("detected", status.y)
  # ) %>%
  lapply(
  ...
  lapply(
    ...
    outer_min_p = 0.0005, #outer_label_colour = "palette", #outer_pattern = "creased",
    # outer_rotate = TRUE, 
    outer_label = "text"
  ) 
donuts <- names(donuts) %>% 
  lapply(\(x) donuts[[x]] + plot_annotation(title = x)) %>% 
  setNames(names(rna))
donuts[[1]]
