...
donuts <- rna %>% 
  ...
  lapply(
    ...
    inner_glue = "{inner}\n{.data[[inner]]}\n{comma(n, 1)}\n{percent(p,0.1)}",
    inner_label_alpha = 0.8, inner_min_p = 0.025,
...
lfc_by_region <- names(rna) %>% 
  lapply(
    \(i) {
      ...
      results %>% 
        as_tibble() %>% 
        dplyr::select(-any_of(c("gene_name"))) %>% 
        unnest(gene_id) %>% 
        ...
        geom_label_repel(
          ...
          data = . %>% 
            ...
            dplyr::slice(1:3, .by = all_of(c("region", "status"))),
      ...
      results %>% 
        ...
        geom_label_repel(
          ...
          data = . %>% 
            ...
            dplyr::slice(1:3, .by = all_of(c("feature", "status"))),
          show.legend = FALSE, size = 3, fontface = "italic"
        ) +
        facet_wrap(~feature) +
        scale_colour_manual(values = pw_pl) +
...
```{r}
any_de <- map_lgl(rna, \(x) any(x$FDR < 0.05))
if (any(any_de)) {
  range_js <- JS(
    "function(values) {
        ...
        rna[[i]] %>% 
          ...
          inner_join(
            as_tibble(results) %>% 
              unnest(gene_id) %>% 
              dplyr::select(
                range, any_of(c("region", "feature")), starts_with("log"),
                FDR = !!sym(fdr_column), status, gene_id
              ),
            by = "gene_id"
          ) %>% 
          dplyr::select(
            starts_with("gene"), ends_with(".x"), logCPM, ends_with(".y"), 
            range, any_of(c("region", "feature")),  
          ) %>% 
          reactable(
            ...
            columns = list(
              ...
              logFC.y = colDef(
                name = "logFC", aggregate = "mean", maxWidth = fs*5,
                format = colFormat(digits = 2),
            ...
            columnGroups = list(
              ...
              colGroup(target, c("logCPM", str_subset(names(.), "\\.y$")))
            ),
...
### DE Genes Associated with Changed `r target`
...
if (any_de & any_diff_sites) {
  ...
  para <- glue(
    ...
    region with detected {target} signal, genes were tested for any association 
    between sites showing evidence of diferential signal and differential expression.
    {comma(length(unique(unlist(diff_windows[c('Up', 'Down')]))))} genes were 
    mapped to at least one site showing differential {target} signal with 
    ...
    of genes where no differential {target} signal was detected. Fisher's 
...
`r if (!any_diff_sites) glue("No sites with differential {target} signal were detected.")`
...
cp <-  htmltools::em(
  glue(
    "
    Differentially Expressed (DE) genes were checked for {target} regions which 
    showed evidence of changed {target} signal. 
    ...
    {target} signal are shown for each gene. The summarised range containing 
    ...
    maximum FDR also shown. If {target} is detected within another gene this 
...
### Volcano Plots By `r target` Status

```{r plot-volcano-by-target, fig.cap = glue("*Volcano plot showing gene expression patterns separated by {target} status. The two most highly ranked genes for up and down regulation are labelled in each panel. Given that some genes may be mapped to multiple {target} sites with different patterns, genes may appear in multiple panels.*")}
...
```{r plot-p-target, fig.height = 5, fig.cap = glue("*P-Values for differential expression partitioned by {target} status. Genes were considered as being mapped to a differential {target} site if one or more peaks was considered as showing evidence of changed signal. No IHW procedure was undertaken using this data.*")}
p <- rnaseq %>% 
  mutate(
    mapped = case_when(
      gene_id %in% unlist(diff_windows[names(diff_windows) %in% c("Up", "Down")]) ~ paste(target, "Changed"),
      gene_id %in% unlist(diff_windows[names(diff_windows) %in% c("Unchanged")]) ~ paste(target, "Unchanged"),
      TRUE ~ paste(target, "Undetected")
    )
  ) %>% 
  ggplot(
...
```{r plot-chip-rna-logfc, eval = nrow(de_genes_db_regions) > 0, echo = nrow(de_genes_db_regions) > 0,  fig.cap = glue("*logFC values for differential {target} sites mapped to DE genes. The most extreme site for each quandrant is labelled showing both the gene and centre of the {target} site. Whilst {target} sites may be mapped to multiple genes, only the most highly ranked site is shown mapped to the most highly ranked DE genes. Points are coloured by differential expression status.*")}
de_genes_db_regions %>% 
  ...
  geom_label_repel(
    ...
    data = . %>% 
      ...
      arrange(desc(abs(ChIP_logFC) + abs(RNA_logFC))) %>% 
      split(.$grp) %>% 
