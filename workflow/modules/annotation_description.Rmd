...
In total the annotations provided in `r basename(basename(config$external$gtf))` contained `r comma(length(gtf$gene))` genes, `r comma(length(gtf$transcript))` transcripts and `r comma(length(gtf$exon))` exons, after restricting the dataset to the autosomes and sex chromosomes, discarding scaffolds, haplotypes and mitochondrial genes.
...
Using the GTF `r basename(basename(config$external$gtf))`, **unique** and **non-overlapping** gene and transcript-centric features were defined using `defineRegions()`, and will be treated as annotated regions throughout the workflow.
...
```{r plot-example, fig.height=7, fig.cap = paste("*12kb region surrounding", collapseGenes(subset(gtf$gene, gene_id == id)$gene_name), "showing all annotated regions.*")}
