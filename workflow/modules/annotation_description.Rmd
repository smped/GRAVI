...
```{r plot-pie-hic, eval = has_hic, echo = has_hic, fig.cap = "*Distribution of interaction pairs using pre-defined genomic regions. Region pairs with an interaction frequency < 5% were grouped into the 'Other' category*"}
hic %>% 
  as_tibble() %>% 
  mutate(
    regions = fct_lump_prop(regions, prop = 0.05)
  ) %>%
  plotPie(
    fill = "regions", min_p = 0.075, category_width = 20, label_size = 3.5,
    cat_glue = "{.data[[fill]]}\n{comma(n, 1)} ({percent(p, 0.1)})",
    cat_adj = 0.1, cat_alpha = 0.7, total_size = 4
  ) +
