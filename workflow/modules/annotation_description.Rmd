...
external_features %>% 
    mutate(w = width) %>% 
    as_tibble() %>% 
    pivot_longer(cols = all_of(names(gene_regions)), names_to = "region", values_to = "ol") %>% 
    dplyr::filter(ol > 0) %>% 
    mutate(
        p = w * ol / 1e3,
        region = factor(region, levels = names(gene_regions)),
        feature = factor(feature, levels = names(feat_col))
    ) %>% 
    plotSplitDonut(
        inner = "feature", outer = "region", scale_by = "p",
        r_centre = 0.6, r_outer = 0.6,
        total_glue = "{comma(N, 1)}kb",
        inner_glue = "{str_wrap(str_to_title(str_replace_all(.data[[inner]],'_', ' ')),10)}\n{percent(p, 0.1)}\n({comma(n, 1)}kb)",
        outer_glue = "{str_wrap(regions[as.character(.data[[outer]])], 10)}\n{percent(p, 0.1)}",
        min_p = 0.01, nudge_r = 0.4,
        inner_palette = feat_col,
        outer_palette = unlist(colours$regions)
    ) %>% 
    .[[1]]
```
