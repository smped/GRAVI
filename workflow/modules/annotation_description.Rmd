...
exclude_ranges %>% 
  ...
  summarise(masked = sum(width), .by = c(seqnames, source)) %>% 
  pivot_wider(names_from = "source", values_from = "masked", values_fill = 0) %>% 
  left_join(as_tibble(sq), by = "seqnames") %>% 
  mutate(
    whitelisted = seqlengths - blacklisted - greylisted,
    seqnames = factor(seqnames, levels = seqlevels(sq))
  ) %>% 
  pivot_longer(ends_with("listed"), names_to = "category", values_to = "bp") %>% 
  ...
  labs(x = "Chromosome", y = "Length (Mb)", fill = "Status")
```
