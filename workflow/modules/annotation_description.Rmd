...
Transcription factor binding motifs (TFBMs) were prepared from `r pander(params$enrichment$motifdb$data_source)` with `r nrow(tf_db)` motifs prepared for downstream analysis.
...
Pairwise correlations between motifs were found during preparation and those where $\rho >= 0.9$ were clustered, giving a final set of `r max(as.integer(tf_db$cluster))` clusters.
