...
Transcription factor binding motifs (TFBMs) were prepared from `r pander(params$motifdb$data_source)` with `r nrow(tf_db)` motifs prepared for downstream analysis.
...
Pairwise correlations between motifs were found during preparation and those where $\rho \geq$ `r params$motifdb$cluster_above` were clustered, giving a final set of `r max(as.integer(tf_db$cluster))` clusters.
