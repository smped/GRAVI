...
```{r external-features}
external_features <- file.path(annotation.path, "features.rds") %>% 
  read_rds()
...
```{r plot-feature-by-region, fig.height = 7, eval = FALSE, echo = FALSE, fig.cap = glue("*The proportion of the ranges provided as external features in the file {basename(config$external$feature)}, and which overlap the gene-centric regions defined above. Values were estimated using the proportion of bases within each feature that overlap each of the regions.*")}
external_features %>% 
  unlist() %>% 
  as_tibble() %>% 
