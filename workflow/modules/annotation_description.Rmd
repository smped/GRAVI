...
txt <- ifelse(
  ...
  paste(
    ...
    "For the purposes below, a DE gene is simply considered as one with an",
...
`r if (length(gsea_dir) > 0) "## Directional GSEA {.tabset}"`
    ...
    lapply(
      \(x) {
        ...
        tbl <- gsea_dir[[x]] %>% 
          ...
          reactable(
            columns = list(
              pathway = colDef(cell = \(value) str_replace_all(value, "_", " ")),
              pval = colDef(show = FALSE),
              padj = colDef(cell = \(value) sprint_pval(value)),
              NES = colDef(format = colFormat(digits = 2))
            ),
            elementId = id
          )
...
`r if (length(gsea_sig) > 0) "## Non-Directional GSEA {.tabset}"`
