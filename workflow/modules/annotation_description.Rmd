...
all_out <- list(
  ...
  motifs = file.path(annotation_path, "motif_list.rds"),
...
In addition, grey-listed regions were derived from all the input samples, `r pander(unique(samples$input))`
...
Transcription factor binding motifs (TFBMs) were prepared from `r pander(params$motif_analysis$motifdb$data_source)` with `r nrow(tf_db)` motifs prepared for downstream analysis.
...
Pairwise correlations between motifs were found during preparation and those where $\rho \geq$ `r params$motif_analysis$motifdb$cluster_above` were clustered, giving a final set of `r max(as.integer(tf_db$cluster))` clusters.
