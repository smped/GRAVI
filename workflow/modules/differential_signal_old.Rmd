...
facet_labeller <- as_labeller(
  c(
    ...
    c(
      Up = glue("Increased {target} Signal"),
      Down = glue("Decreased {target} Signal")
    ),
    c(
...
all_out <- list(
  results = file.path(
    out_path, 
    glue("{target}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_signal.rds")
  ),
  csv = file.path(
    out_path, 
    glue("{target}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_signal.csv.gz")
  ),
  ...
  renv = here::here(
    "output/envs",
    glue(
      "{target}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_signal.RData"
    )
  )
)
...
3. The *limma-trend* method [@LawVoom2014] is used alongside `treat` [@McCarthyTreat2009] for detection of differential signal. This should correctly control the FDR, and allows the specification as a percentage of a suitable threshold for the estimated change in signal, below which we are not interested in any changed signal
    "
  ),
  "ls-ql" = glue(
    ...
    3. A range-based Null Hypothesis [@McCarthyTreat2009] will be used for testing of differential signal as described in detail below"
  )
)
desc_ihw <- list(
...
This step of the workflow uses a sliding window approach to differential signal, as a logical extension to the methods suggested and implemented in the `csaw` package [@LunSmythF1000].
...
The workflow also depends heavily on the functions implemented in the Bioconductor package [extraChIPs](https://bioconductor.org/packages/release/bioc/html/extraChIPs.html)
...
2. Genes mapped to *all windows with differential signal* are compared to genes mapped to windows with no difference in signal
3. Genes mapped to windows with *increasing* `r target` signal are compared to genes mapped to windows with no difference in signal
4. Genes mapped to windows with *decreasing* `r target` signal are compared to genes mapped to windows with no difference in signal
...
`r if (has_rnaseq) "Any association between differentially expressed genes and differential signal regions will be assessed using Gene Set Enrichment Analysis [@SubramanianGsea2005], as implemented in the fgsea package [@KorotkevichFgsea2019]. The sets of genes associated with changed signal will be subset by regions and any provided external features, and these novel gene-sets will be used to test for enrichment within the RNA-Seq results. ChIP-seq derived gene-sets will be tested for differential expression using genes ranked directionally and by significance alone. A combined meta-analytic approach using Wilkinson's method will also be used to integration of results from enrichment testing in each standalong dataset."`
...
A set of genomic sliding windows were defined previously using a window size of `r win_size`bp, sliding in increments of `r win_step`bp.
With the exclusion of black-listed and grey-listed regions, all alignments within each window were counted for each `r target`-associated sample, and all relevant input samples.
Any windows with fewer than `r ncol(filtered_counts)` alignments (i.e.1 read/sample) across all samples were initially discarded before filtering using `extraChIPs::dualFilter()`

When filtering, windows were discarded with low counts using a) counts relative to the Input sample and b) the overall signal level.
Thresholds were set automatically to include `r percent(config$comparisons$filter_q)` of windows which overlapped the reference set of sites, defined as those identified by `macs callpeak` across all treatments.
This ensures a minimum number of counts was present within each window, specifically avoiding problems created across regions where no counts were returned in any input samples.
...
list(
  ...
  pander(
    ...
    caption = paste(
      ...
      "whilst those retained after filtering were focussed on strong signal.",
      "Union peaks were as identified by `macs2 callpeak` in a", 
...
H_A: |\mu| > \lambda
$$
where $\mu$ represents the true mean change in `r target` signal.
...
For subsequent classification when combining across multiple differential signal results, an additional p-value testing $\mu = 0$ was added, however this is not used for detection of sites showing evidence of altered `r target` signal.
...
Representative estimates of differential signal (i.e. logFC) were taken from the sliding window with the *highest average signal* across all samples, within each set of windows to be merged.
...
The number of windows showing increased or decreased signal within each merged region were also included, by counting those within each set of windows with p-values lower than the selected window.
...
cp <- htmltools::em(
  glue(
    ...
    comma(sum(mcols(merged_results)[[fdr_column]] < fdr_alpha)), 
    ...
    function(x) {
      glue(
        ...
        *Heatmap and histogram for all regions considered to show evidence of 
        {ifelse(x == 'Up', 'increased', 'decreased')} {target} signal in 
...
`r ifelse("Up" %in% names(profile_heatmaps), "#### Heatmap: Increased Signal\n", "")`
...
`r ifelse("Down" %in% names(profile_heatmaps), "#### Heatmap: Decreased Signal\n", "")`
...
#### Differential Signal By Region

```{r plot-db-regions, fig.height = 7, fig.cap = glue("*Merged windows considered as showing differential {target} signal across all genomic regions.*")}
merged_results %>% 
  subset(status != "Unchanged") %>%
  as_tibble() %>% 
  dplyr::rename(
    "{target}" := status, Region = region
  ) %>% 
  plotSplitDonut(
...
merged_results %>% 
  ...
  as_tibble() %>% 
  dplyr::rename(
    "{target}" := status, Feature = feature
  ) %>% 
  plotSplitDonut(
    inner = target, outer = "Feature", min_p = 0.01,
    inner_glue = "{.data[[inner]]}\nn = {comma(n, 1)}\n{percent(p, 0.1)}",
    outer_glue = "{str_wrap(.data[[outer]], 12)}\nn = {comma(n, 1)}\n{percent(p, 0.1)}",
    inner_palette = direction_colours,
...
`r ifelse(has_features, "#### Differential Signal By Region And Feature", "")`

```{r scatterpie-region-feature, eval = has_features, echo = has_features, fig.cap = glue("*Comparison of changes in {target} signal separated by gene-centric region and external features provided in {basename(config$external$features)}*")}
df <- merged_results %>% 
  as_tibble() %>% 
...
y_lim <- list2(
  "{target}" := here::here(
    "output", "macs2", target, "{treat_levels}_merged_treat_pileup.summary"
  ) %>% 
    glue() %>% 
    lapply(read_tsv) %>% 
    bind_rows() %>% 
...
```{r plot-qsmooth-top-ranges, eval = db_method == "sq-lt", fig.height = fh, echo = db_method == "sq-lt", fig.cap = glue("*Most highly ranked ranges for both gained and decreased {target} signal in repsonse to {treat_levels[[2]]} treatment. The smooth-quantile normalised values are shown across the initial set of sliding windows before merging. Ranges were chosen as the most extreme for FDR, Signal Strength and logFC. Windows are shown free of the genomic context.*")}
grl_to_plot %>% 
  lapply(
    function(x){
      filtered_counts %>% 
...
`r ifelse("top_down_by_expr" %in% names(grl_to_plot), "#### Decreased Signal (Strongest Signal)", "")`
...
`r ifelse("top_down_by_fdr" %in% names(grl_to_plot), "#### Decreased Signal (Lowest FDR)", "")`
...
`r ifelse("top_down_by_lfc" %in% names(grl_to_plot), "#### Decreased Signal (Most Extreme)", "")`
...
`r ifelse("top_down_no_macs2" %in% names(grl_to_plot), "### Significant Decreased Signal (No Union Peak)", "")`
...
`r ifelse("top_up_by_expr" %in% names(grl_to_plot), "#### Increased Signal (Strongest Signal)", "")`
...
`r ifelse("top_up_by_fdr" %in% names(grl_to_plot), "#### Increased Signal (Lowest FDR)", "")`
...
`r ifelse("top_up_by_lfc" %in% names(grl_to_plot), "#### Increased Signal (Most Extreme)", "")`
...
`r ifelse("top_up_no_macs2" %in% names(grl_to_plot), "### Significant Increased Signal (No Union Peak)", "")`
