...
cmn_diff <- rnaseq_gsea %>%
  ...
  unnest(p) %>% 
  mutate(fdr = p.adjust(pval, params$enrichment$adj)) %>% 
  arrange(pval) %>% 
  ...
  mutate(
    ...
    Status = case_when(
      padj < enrich_alpha & adj_p < enrich_alpha ~ "Both",
      padj < enrich_alpha & !adj_p < enrich_alpha ~ "RNA Only",
      !padj < enrich_alpha & adj_p < enrich_alpha ~ "ChIP Only",
      !padj < enrich_alpha & !adj_p < enrich_alpha ~ "Neither"
    )
  ) %>% 
  dplyr::select(
    gs_name = pathway, fdr, Status, NES, padj, adj_p,
    starts_with("num"), leadingEdge
  ) 
tg_diff <- cmn_diff %>% 
  make_tbl_graph(gs_by_gsid)
...
tg_diff %>% 
  ...
  geom_node_point(
    aes(fill = -log10(fdr), size = numDEInCat),
    shape = 21
  ) +
    geom_node_text(
  ...
  reactable(
    ...
    columns = list(
      ...
      NES = colDef(
        format = colFormat(digits = 2),
        style = function(value) {
          col <- ifelse(value > 0, colours$direction["down"], colours$direction["down"])
          list(color = col)
        },
        maxWidth = 80
      ),
