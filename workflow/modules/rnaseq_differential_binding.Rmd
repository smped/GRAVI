    ...
    columns = c(
      list(
        ...
        ChIP_logCPM = colDef(
          ...
          aggregate = range_js,
          format = colFormat(digits = 2),
          style = function(value) bar_style(width = value / max(merged_results$AveExpr))
        ),
        ChIP_logFC = colDef(
          name = paste0("logFC (", target, ")"),
          aggregate = "mean",
          maxWidth = 80,
          format = colFormat(digits = 2),
...
cmn_gs <- intersect(
  dplyr::filter(goseq_diff, fdr < fdr_alpha)$gs_name,
  dplyr::filter(rnaseq_gsea, padj < fdr_alpha)$pathway
)
```
...
cmn_gs <- intersect(
  dplyr::filter(goseq_up, fdr < fdr_alpha)$gs_name,
  dplyr::filter(rnaseq_gsea, padj < fdr_alpha)$pathway
)
```
...
cmn_gs <- intersect(
  dplyr::filter(goseq_down, fdr < fdr_alpha)$gs_name,
  dplyr::filter(rnaseq_gsea, padj < fdr_alpha)$pathway
)
```
