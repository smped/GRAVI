...
```{r plot-region-feature-detected, eval = has_features, echo = has_features, fig.cap = glue("*Distribution of {target}-bound windows mapped to detected/undetected genes separated by both gene-centric-region and external fatures.*")}
merged_results %>% 
  mutate(
    any_detected = ifelse(
      vapply(detected, length, integer(1)) > 1,
