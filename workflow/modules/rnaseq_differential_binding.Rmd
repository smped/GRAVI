...
```{r plot-volcano-by-target, fig.cap = glue("*Volcano plot showing gene expression patterns separated by {target} status. The two most highly ranked genes for up and down regulation are labelled in each panel. Given that some genes may be mapped to {target} binding events which include both an increase and decrease in binding, genes are simply grouped by whether any differential binding was mapped to an associated regulatory region.*")}
df <- rnaseq %>% 
  mutate(
    mapped = case_when(
...
```{r plot-p-target, fig.height = 6, fig.cap = glue("*P-Values for differential expression partitioned by {target} ChIP peak status. Genes were considered as being mapped to a differentially bound peak if one or more peaks was considered as differentially bound. No IHW procedure was undertaken using this data.*")}
p <- rnaseq %>% 
  mutate(
    mapped = case_when(
      gene_id %in% unlist(diff_windows[names(diff_windows) %in% c("Up", "Down")]) ~ paste(target, "Differentially Bound"),
...
## Integration With Gene Expression GSEA
...
txt <- ifelse(
  length(cmn_gs) == 0,
  glue("No gene sets were shared between all {target} differentially bound windows and the RNA-Seq data"),
  "#### Results Table"
)
plot_net <- length(cmn_gs) >= min_network_size
```
...
cp <- htmltools::tags$em(
  glue(
    ...
    as ranked in the provided RNA-Seq dataset, subset to only include those 
    with a mapped site showing change in {target} signal.
    "
  )
)
tbl <- goseq_diff %>% 
  ...
  mutate(
    ...
    leadingEdge = lapply(leadingEdge, intersect, unlist(diff_windows[c("Up", "Down")])),
    leadingEdge = vapply(
      leadingEdge,
...
```{r network-cmn-alldiff, eval = plot_net, fig.cap = glue("*Network plot for pathways considered significant when assessing all differentially bound {target} sites and which were also considered significant when performing GSEA on the provided RNA-Seq data. Distances are derived from genes within the leading edge for each pathway and which are also {target} targets with a differentially bound site, as per the results table. Only pathways with a distance < 0.9 will be connected as nodes, width edge widths also indicating the degree of overlap (or distance) between pathways.*")}
rnaseq_gsea %>% 
  dplyr::filter(pathway %in% cmn_gs) %>% 
  dplyr::select(
    gs_name = pathway, numInCat = size, leadingEdge, pval, adj_p = padj
  ) %>% 
  mutate(
  ...
  make_tbl_graph(
    gs = setNames(.$leadingEdge, .$gs_name)
  )%>% 
...
txt <- ifelse(
  length(cmn_gs) == 0,
  glue("No gene sets were shared between {target} bound windows showing increased binding and the RNA-Seq data"),
  "#### Results Table"
)
plot_net <- length(cmn_gs) >= min_network_size
```
...
cp <- htmltools::tags$em(
  glue(
    ...
    the genes as ranked in the provided RNA-Seq dataset, subset by those with 
    a mapped site showing {target} increase.
    "
  )
)
tbl <- goseq_diff %>% 
  ...
  mutate(
    edge_size = vapply(leadingEdge, length, integer(1)),
    leadingEdge = lapply(leadingEdge, intersect, diff_windows$Up),
    leadingEdge = vapply(
      leadingEdge,
      function(x) paste(id2gene[x], collapse = "; "),
      character(1)
...
```{r network-cmn-up, eval = plot_net, fig.cap = glue("*Network plot for pathways considered significant when assessing {target} sites with increased binding and which were also considered significant when performing GSEA on the provided RNA-Seq data. Distances are derived from genes within the leading edge for each pathway and which are also {target} targets with a differentially bound site, as per the results table. Only pathways with a distance < 0.9 will be connected as nodes, width edge widths also indicating the degree of overlap (or distance) between pathways.*")}
rnaseq_gsea %>% 
  dplyr::filter(pathway %in% cmn_gs) %>% 
  dplyr::select(
    gs_name = pathway, numInCat = size, leadingEdge, pval, adj_p = padj
  ...
  mutate(
    leadingEdge = lapply(
      leadingEdge, 
      function(x) id2gene[x] %>% intersect(
        unique(unlist(subset(merged_results, status == "Up")$gene_name))
      )
    ),
    numDEInCat = vapply(leadingEdge, length, integer(1))
  ) %>% 
...
txt <- ifelse(
  length(cmn_gs) == 0,
  glue("No gene sets were shared between {target} bound windows showing decreased binding and the RNA-Seq data"),
  "#### Results Table"
)
plot_net <- length(cmn_gs) >= min_network_size
```
...
cp <- htmltools::tags$em(
  glue(
    ...
    found in both analyses are detailed below. Leading Edge genes represent 
    the genes as ranked in the provided RNA-Seq dataset, subset by those with 
    mapped {target} site showing a decrease in signal.
    "
  )
)
tbl <- goseq_diff %>% 
  ...
  mutate(
    edge_size = vapply(leadingEdge, length, integer(1)),
    leadingEdge = lapply(leadingEdge, intersect, unlist(diff_windows[c("Down")])),
    leadingEdge = vapply(
      leadingEdge,
      function(x) paste(id2gene[x], collapse = "; "),
      character(1)
...
```{r network-cmn-down, eval = plot_net, fig.cap = glue("*Network plot for pathways considered significant when assessing {target} sites with decreased binding and which were also considered significant when performing GSEA on the provided RNA-Seq data. Distances are derived from genes within the leading edge for each pathway and which are also {target} targets with a differentially bound site, as per the results table. Only pathways with a distance < 0.9 will be connected as nodes, width edge widths also indicating the degree of overlap (or distance) between pathways.*")}
rnaseq_gsea %>% 
  dplyr::filter(pathway %in% cmn_gs) %>% 
  dplyr::select(
    gs_name = pathway, numInCat = size, leadingEdge, pval, adj_p = padj
  ...
  mutate(
    leadingEdge = lapply(
      leadingEdge, 
      function(x) id2gene[x] %>% intersect(
        unique(unlist(subset(merged_results, status == "Down")$gene_name))
      )
    ),
    numDEInCat = vapply(leadingEdge, length, integer(1))
  ) %>% 
