...
if (any_de & any_diff_sites) {
  ft_df <- rnaseq %>%
    ...
    mutate(
      window = case_when(
        gene_id %in% unlist(diff_windows[c("Up", "Down")]) ~ paste(target, "Differentially Bound"),
        TRUE ~ "Unchanged"
      ) %>%
        fct_infreq()
    ) %>% 
    group_by(DE, window) %>% 
    tally() %>% 
    pivot_wider(names_from = "DE", values_from = "n", names_prefix = "DE_") 
  ft <- ft_df%>% 
    as.data.frame() %>% 
  ...
  para <- glue(
    ...
    {comma(ft_df$DE_TRUE[2])} (
    {percent(ft_df$DE_TRUE[2] / sum(unlist(ft_df[2, 2:3])), 0.1)}) of these 
    being considered differentially expressed. This compares to 
    {percent(ft_df$DE_TRUE[1] / sum(unlist(ft_df[1, 2:3])), 0.1)} of genes
    where no differential {target} binding was detected. Fisher's Exact test
    for any association gave this a p-value of {sprintf('%.2e', ft$p.value)}.
    "
  )
}
```
