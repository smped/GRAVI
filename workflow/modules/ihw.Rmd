...
if (ihw_method == "targets") {
  ...
  other_targets <- here::here(config$samples$file) %>% 
    ...
    unique() %>% 
    setdiff(target)
  ihw_gr <- other_targets %>% 
    map_chr(
      \(x) file.path(all_paths$peaks, x, glue("{x}_consensus_peaks.bed.gz"))
    ) %>% 
...
results <- addDiffStatus(results, sig_col = fdr_column, alpha = params$alpha)
```
...
ihw %>%
  ...
  mutate(
    ...
    ihw_status = case_when(
      fdr < params$alpha & adj_pvalue > params$alpha ~ "Lost",
      fdr < params$alpha & adj_pvalue < params$alpha ~ "Retained",
      fdr > params$alpha & adj_pvalue < params$alpha ~ "Gained",
      fdr > params$alpha & adj_pvalue > params$alpha ~ "Never Significant"
    ) %>% 
      factor(
...
ihw %>%
  ...
  geom_hline(
    ...
    data = . %>% 
      dplyr::select(-group) %>% 
      dplyr::filter(adj_pvalue < params$alpha) %>% 
  ...
  geom_vline(
    ...
    data = . %>% 
      dplyr::filter(p.adjust(pvalue, "fdr") < params$alpha) %>% 
