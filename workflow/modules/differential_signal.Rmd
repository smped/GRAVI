...
```{r plot-ma, fig.cap = glue("*MA plot showing the status of all {region_type} under consideration. The two most extreme {region_type} are labelled by region and any associated genes, whilst the overall pattern of association between signal level (logCPM) and changed signal (logFC) is shown as the blue curve.*")}
...
if (has_rna) {
  rna_status <- rna %>% 
    ...
    mutate(status = factor(
      status, levels = c(names(direction_colours), "Undetected"))
    ) %>% 
    arrange(status) %>% 
