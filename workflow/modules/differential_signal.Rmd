...
tbl_cols <- list(
  Region = colDef(
    ...
    style = \(value) {
      args <- as.list(c(col2rgb(region_colours[[value]])[,1]/255, alpha = 0.7))
      bc <- do.call("rgb", args)
      list(borderRight = "1px solid rgba(0, 0, 0, 0.1)", backgroundColor = bc)
    }
  ),
  feature = colDef(
    "Feature", footer = htmltools::tags$b("Total"), minWidth = 180,
    style = \(value) {
      args <- as.list(c(col2rgb(colours$features[[value]])[,1]/255, alpha = 0.7))
      bc <- do.call("rgb", args)
      list(borderRight = "1px solid rgba(0, 0, 0, 0.1)", backgroundColor = bc)
    }
  ),
  Increased = colDef(
    format = comma_col, maxWidth = 100,
    style = \(value) {
...
if (has_features) {
  ...
  tbl <- df %>%
    reactable(
      ...
      fullWidth = TRUE, elementId = 'tbl-results-by-feature'
    )
  tl <- tagList(tbl, cp)
  if (interactive()) {
    htmltools::browsable(tl)
...
In addition to enrichment of pathways, the `r region_type` showing changed signal were assessed for their relationship to the pre-defined gene-regions`r ifelse(has_features, " and the supplied external features.", ".")`.
...
```{r zscore-heatmap, fig.cap = glue("*Normalised Z-scores showing the effect of shifting {target} peaks within a $\\pm$ {regioner_params$window/1e3}kb window around the existing peaks.*")}
shifted_z %>% 
  arrange(abs(shift), Z) %>% 
  mutate(
