...
```{r dummy-params, eval = interactive(), echo = FALSE}
params <- list(
  target = "ER",
...
motif_uri <- file.path(all_paths$annotation, "motif_uri.rds") %>% 
  read_rds()
motif_enrich_res <- all_paths$diff_signal %>% 
  file.path(target, "{comp}_motif_enrichment.tsv.gz") %>% 
  glue() %>% 
  read_tsv() %>%
  dplyr::rename(adj_p = fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_params$adj)) %>% 
  dplyr::filter(adj_p < motif_params$alpha, enrichment > 1)
motif_pos_res <- all_paths$diff_signal %>% 
  file.path(target, "{comp}_motif_position.tsv.gz") %>% 
  glue() %>% 
  read_tsv() %>%
  dplyr::rename(adj_p = fdr) %>% 
  mutate(adj_p = p.adjust(p, motif_params$adj)) %>% 
