...
```{r ma-by-feature, echo = has_features, fig.cap = glue("*MA plot showing the status of all {region_type} under consideration. The two most extreme {region_type} are labelled by region and any associated genes, whilst the overall pattern of association between signal level (logCPM) and changed signal (logFC) is shown as the blue curve. Boxplots summarise the distribution of values for both expression and logFC by which feature best characterises the {region_type}*"*")}
if (has_features) {
  results %>% 
    as_tibble() %>%
    ggplot(aes(logCPM, logFC)) +
