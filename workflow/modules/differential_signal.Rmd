...
id2gene <- structure(gtf_gene$gene_name, names = gtf_gene$gene_id) %>% 
  .[!duplicated(names(.))]
genes_to_show <- subset(gtf_gene, gene_type == "protein_coding")$gene_name
...
df <- results %>%
  ...
  mutate(
    other_genes = lapply(Genes, setdiff, genes_to_show), 
    Genes = lapply(Genes, intersect, genes_to_show)
  )
tbl <- df %>% 
  reactable(
...
results %>% 
  ...
  geom_label_repel(
    ...
    data = . %>%
      ...
      mutate(
        detected = gene_name %>%
          lapply(intersect, genes_to_show) %>% 
...
```{r add-nfr, eval = length(nfr) > 0,  child = here::here('workflow/modules/_nfr_differential_signal.Rmd')}
```
