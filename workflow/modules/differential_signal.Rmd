...
diff_sig_params <- here::here("config", "json", "differential_signal_param.json") %>% 
  jsonlite::fromJSON() %>% 
  pluck(target)
win_type <- match.arg(diff_sig_params$window_type, c("fixed", "sliding"))
win_size <- diff_sig_params$window_size
method <- diff_sig_params$method
norm <- match.arg(
  diff_sig_params$norm,
  c(eval(formals(calcNormFactors.default)$method), "sq")
)
pair_col <- diff_sig_params$pair_column ## This may need revision of earlier modules
fdr_alpha <- diff_sig_params$alpha
fc <- diff_sig_params$fc
ihw_method <- match.arg(
  diff_sig_params$ihw, c("regions", "features", "targets", "none")
)
...
samples <- file.path(all_paths$macs2, glue("{target}_qc_samples.tsv")) %>% 
  read_tsv() %>% 
...
blacklist <- here::here(all_paths$annotation, "blacklist.rds") %>% 
  read_rds()
grey_path <- here::here("output", "greylist")
greylist <- GRanges(seqinfo = sq)
if (length(unique(samples$input))) {
  greylist <- file.path(grey_path, "{samples$input}_greylist.bed") %>% 
    unique() %>% 
    glue() %>% 
...
```{r load-counts}
counts <- here::here("output", "counts", "{target}_counts.rds") %>% 
  glue() %>% 
  read_rds()
...
tbl <- results %>%
  ...
  plyranges::select(
    w, starts_with("log"), !!sym(fdr_column), 
...
```{r make-heatmaps}
profile_width <- ifelse(str_to_lower(target) == "h3k27ac", 1e4, 5e3)
n_bins <- 100
bwfl <- file.path(
  all_paths$macs2, glue("{target}_{treat_levels}_merged_treat_pileup.bw")
) %>% 
  BigWigFileList() %>% 
...
sp <- case_when(
  str_detect(config$genome$build, "GRCh|hg|CHM|T2T") ~ "Homo sapiens",
  str_detect(config$genome$build, "GRCm|mm") ~ "Mus musculus",
  str_detect(config$genome$build, "Dan|GRCz|z[0-9]") ~ "Danio rerio",
  str_detect(config$genome$build, "mRat|Rnor") ~ "Rattus norvegicus",
  str_detect(config$genome$build, "GRCg|Gallus") ~ "Gallus gallus",
  TRUE ~ NA_character_
)
if (is.na(sp)) stop("Unsupported genome")
