...
```{r zscore-heatmap, eval = FALSE, fig.cap = glue("*Normalised Z-scores showing the effect of shifting {target} peaks within a $\\pm$ {slot(mlz, 'parameters')$window / 1e3}kb window around the existing peaks.*")}
shifted_z %>% 
  arrange(abs(shift), Z) %>% 
  mutate(
