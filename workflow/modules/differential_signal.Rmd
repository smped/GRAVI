...
n_changed <- sum(results$status %in% c("Increased", "Decreased"))
region_type <- ifelse(
...
tbl_cols <- list(
  ...
  `% Changed` = colDef(
    ...
    align = "right",
    footer = htmltools::tags$b(percent(sum(1 - n_unch / n_windows), 0.1))
  ),
  Total = colDef(
    format = comma_col, maxWidth = 100,
    style = \(value) {
  ...
  dplyr::select(
    ...
    bplapply(
      \(x) {
        m <- matrix(
          c(
            x$Total - x$Unchanged, x$Unchanged, 
            n_changed - (x$Total - x$Unchanged),
            n_unch - x$Unchanged
          ), 
          nrow = 2
        )
        bind_cols(x, fisher.test(m)[c("p.value", "estimate")])
      }
    ) %>% 
    bind_rows() %>% 
    mutate(
      ...
      Association = case_when(
        FDR < 0.05 & estimate > 1 ~ "Changed",
        FDR < 0.05 & estimate < 1 ~ "Unchanged",
        TRUE ~ NA_character_
      )
    ) %>% 
    arrange(desc(Total)) %>% 
    dplyr::select(-all_of(c("p.value", "estimate")))
cp <- htmltools::em(
  glue(
    ...
    "Fisher's Exact Test (two-sided) was performed to identify any association ",
    "between a site being changed or unchanged within each region, with ",
    "FDR-adjusted p-values and any detected association given int he final columns."
  )
)
tbl <- df %>%
  reactable(
...
if (has_features) {
  ...
  cp <- htmltools::em(
    glue(
      ...
      "Fisher's Exact Test (two-sided) was performed to identify any association ",
      "between a site being changed or unchanged when overlapping a given feature, with ",
      "FDR-adjusted p-values and any detected association given int he final columns. ",
      "Given that {str_to_lower(region_type)} may overlap multiple features, ",
      "no column totals are provided"
    )
  )
  tbl <- df %>%
    reactable(
  ...
  cp <- c(
    glue(
      ...
      ifelse(
        has_features,
        glue(
          "Using the features supplied this overlapped ",
          glue_collapse(feat, sep = ", ", last = " & "),
...
mlz <- all_paths$diff_signal %>% 
  file.path("{target}", "{comp}-regions_localz.rds") %>% 
  glue() %>% 
  read_rds()
localz_df <- slot(mlz, "multiLocalZscores")$resumeTab %>%
  ...
  mutate(
    ...
    prop = n_overlaps / length(results)
  ) 
