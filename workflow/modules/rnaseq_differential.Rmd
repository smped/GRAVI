...
The distribution of merged windows, where `r target` signal was detected, was firstly compared to genes within the RNA-Seq dataset.
...
### Detected Genes By `r target` Signal

```{r plot-region-detected, fig.height = 6, fig.cap = glue("*Distribution of {target} windows by region, according to whether the window is mapped to a detected gene in the RNA-Seq dataset.*")}
merged_results %>%
  mutate(
    RNASeq = ifelse(
      vapply(gene_name, length, integer(1)) > 1, "Detected", "Undetected"
    )
  ) %>% 
  as_tibble() %>% 
  dplyr::rename(Region = region) %>%   
  plotSplitDonut(
      inner = "RNASeq", outer = "Region", min_p = 0.02, r_outer = 0.7,
      label_alpha = 0.7,
      inner_glue = "{.data[[inner]]}\n{percent(p, 0.1)}\nn = {comma(n, 1)}",
      outer_glue = "{str_wrap(.data[[outer]], 12)}\n{percent(p, 0.1)}\nn = {comma(n, 1)}",
      inner_palette = as.character(colours$qc),
...
```{r plot-feature-detected, eval = has_features, echo = has_features, fig.height = 6, fig.cap = glue("*Distribution of {target} windows by externally-defined features from {basename(config$external$features)}, according to whether the window is mapped to a detected gene in the RNA-Seq dataset.*")}
merged_results %>%
  mutate(
    RNASeq = ifelse(
      vapply(gene_name, length, integer(1)) > 1, "Detected", "Undetected"
  ...
  plotSplitDonut(
      inner = "RNASeq", outer = "Feature", min_p = 0.02, r_outer = 0.7,
      label_alpha = 0.7,
      inner_glue = "{.data[[inner]]}\n{percent(p, 0.1)}\nn = {comma(n, 1)}",
      outer_glue = "{str_wrap(.data[[outer]], 12)}\n{percent(p, 0.1)}\nn = {comma(n, 1)}",
      inner_palette = as.character(colours$qc),
...
```{r barcode-gsea-dir-sig, eval = nrow(gsea_dir_sig) > 0, echo = nrow(gsea_dir_sig) > 0, fig.height= max(3, 3 * ceiling( length(p) / 3)), fig.width = 1 + min(3*length(p), 9), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __directional__ changes in gene expression.*")}
...
```{r barcode-gsea-reg-dir-sig, eval = nrow(gsea_reg_dir_sig) > 0, echo = nrow(gsea_reg_dir_sig) > 0, fig.height= max(3, 3 * ceiling( length(p) / 3)), fig.width = 1 + min(3*length(p), 9), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __directional__ changes in gene expression.*")}
plot_grid(plotlist = p, nrow = ceiling(length(p) / 3))
```
...
```{r barcode-gsea-feat-dir-sig, eval = has_features & nrow(gsea_feat_dir_sig) > 0, echo = has_features & nrow(gsea_feat_dir_sig) > 0,fig.height= max(3, 3 * ceiling( length(p) / 3)), fig.width = 1 + min(3*length(p), 9), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __directional__ changes in gene expression.*")}
plot_grid(plotlist = p, nrow = ceiling(length(p) / 3))
```
...
```{r barcode-gsea-nondir-sig, eval = nrow(gsea_nondir_sig) > 0, echo = nrow(gsea_nondir_sig) > 0, fig.height= max(3, 3 * ceiling( length(p) / 3)), fig.width = 1 + min(3*length(p), 9), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __non-directional__ changes in gene expression.*")}
plot_grid(plotlist = p, nrow = ceiling(length(p) / 3))
```
...
```{r barcode-gsea-reg-nondir-sig, eval = nrow(gsea_reg_nondir_sig) > 0, echo = nrow(gsea_reg_nondir_sig) > 0, fig.height= max(3, 3 * ceiling( length(p) / 3)), fig.width = 1 + min(3*length(p), 9), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __non-directional__ changes in gene expression.*")}
plot_grid(plotlist = p, nrow = ceiling(length(p) / 3))
```
...
```{r barcode-gsea-feat-nondir-sig, eval = nrow(gsea_feat_nondir_sig) > 0, echo = nrow(gsea_feat_nondir_sig) > 0, fig.height= max(3, 3 * ceiling( length(p) / 3)), fig.width = 1 + min(3*length(p), 9), fig.cap = glue("*Barcode plots for the top {length(p)} sets of windows associated with __nondirectional__ changes in gene expression.*")}
plot_grid(plotlist = p, nrow = ceiling(length(p) / 3))
```
