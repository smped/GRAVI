...
All combinations of the comparisons defined above will will be used to perform pairwise comparisons.
...
`r ifelse(length(ext_data) > 0, "The external data provided was:", "No external data was included")`
...
- For a sample to pass initial QC, the number of identified peaks needs to be at least `r percent(config$peaks$qc$min_prop_peaks)` of the number of peaks peaks identified in the sample with the highest number of peaks for each specific treatment group, assuming this represents the *best* sample in that group. 
In many instances, no peaks will be returned in any condition and in this situation all samples will be retained. However, samples significantly diverging from the 'best' sample will be excluded.
