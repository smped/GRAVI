...
out_path <- here::here("output", "differential_h3k27ac", target)
...
all_out <- list(
  results = file.path(
    out_path, 
    glue("{target}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_h3k27ac.rds")
  ),
  csv = file.path(
    out_path, 
    glue("{target}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_h3k27ac.csv.gz")
  ),
  ...
  renv = here::here(
    "output/envs",
    glue(
      "{target}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_h3k27ac.RData"
    )
  )
)
...
filtered_counts <- read_rds(
    here::here(out_path, glue("{target}_filtered_counts.rds"))
) %>% 
    subset(select = treat %in% treat_levels) %>% 
    droplevels()
stopifnot(ncol(filtered_counts) == nrow(samples))
...
Thresholds were set automatically to include `r percent(config$comparisons$filter_q)` of windows which overlapped the reference set of binding sites, defined as those identified by `macs callpeak` across all treatments.
...
profile_heatmaps <- sig_profiles %>% 
  bplapply(
    ...
    fillLab = "logCPM",
    BPPARAM = MulticoreParam(length(sig_profiles))
  ) 
...
df <- tibble(gene_id  = unique(genes_gr$gene_id)) %>% 
...
tbl <- goseq_diff %>% 
  ...
  dplyr::select(
    ...
    `Genes Mapped to Changing Windows` = gene_name
  ) %>% 
  reactable(
    filterable = TRUE,
    showPageSizeOptions = TRUE,
    columns = list(
      ...
      numDEInCat = colDef(name = "Total Mapped To Changing Windows", maxWidth = 80),
      numInCat = colDef(
        name = "Total Mapped",
        maxWidth = 80,
        cell = function(value) comma(value, 1)
      ...
      "Genes Mapped to Changing Windows" = colDef(
        cell = function(value) with_tooltip(value, width = 100),
        minWidth = 200
      )
    )
