...
## Default sliding windows are about 2/3 of the fragment length
win_step <- 10*(1 + fl %/% 60)
win_size <- 3*win_step
## Overwrite the defaults only if specified
if (is.numeric(config$comparisons$windows$step)) 
    win_step <- config$comparisons$windows$step
if (is.numeric(config$comparisons$windows$size))
