  ...
  lapply(
    function(x) {
      vals <- rbind(x$c1, x$c2) %>% 
        colMeans()
      rgb <- rgb(vals["red"], vals["green"], vals["blue"], maxColorValue = 255)
  ...
  join_overlap_left(
    db_results[[targets[[1]]]] %>% 
      select(
        ...
        !!sym(glue("{targets[[1]]}_fdr")) := fdr_ihw
      )
  ) %>%
  join_overlap_left(
    db_results[[targets[[2]]]] %>% 
      select(
        ...
        !!sym(glue("{targets[[2]]}_fdr")) := fdr_ihw
      )
  ) %>% 
  mutate(
    "{targets[[1]]}_status" := fct_explicit_na(
      !!sym(glue("{targets[[1]]}_status")), na_level = "Undetected"
    ),
    "{targets[[2]]}_status" := fct_explicit_na(
      !!sym(glue("{targets[[2]]}_status")), na_level = "Undetected"
    )
  ) %>% 
  as_tibble() %>% 
  mutate(
    status = fct_cross(
      ...
      fct_relabel(
        str_replace_all,
        pattern = "(.+):(.+)",
        replacement = glue("{targets[[1]]} \\1 - {targets[[2]]} \\2")
      ),
    "{targets[[1]]}_status" := paste(
      targets[[1]], !!sym(glue("{targets[[1]]}_status"))
    ),
    "{targets[[2]]}_status" := paste(
      targets[[2]], !!sym(glue("{targets[[2]]}_status"))
    )
  ) %>% 
...
tbl %>% 
  ggplot(
    aes(!!sym(glue("{targets[[1]]}_logFC")), !!sym(glue("{targets[[2]]}_logFC")))
  ) +
  ...
  facet_grid(
    as.formula(
      glue("fct_rev({targets[[2]]}_status) ~ {targets[[1]]}_status")
    )
  ) +
  scale_colour_manual(
    values = direction_colours[levels(tbl$status)]
  )
## Now add gene labels
```
