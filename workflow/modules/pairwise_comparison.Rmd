...
def_cols <- c("sample", "treat", "target", "input", "label", "qc", "type")
...
db_results <- seq_along(pairs) %>% 
  lapply(
    function(x) {
      tg <- names(pairs)[[x]]
      type <- unique(dplyr::filter(samples, target == tg)$type)
      file_type <- "signal"
      if (type == "H3K27ac") file_type <- "h3k27ac"
      here::here(
        "output", glue("differential_{file_type}"), tg, 
        glue("{tg}_{pairs[[x]][[1]]}_{pairs[[x]][[2]]}-differential_{file_type}.rds")
      )
    }
  ) %>% 
  lapply(read_rds) %>% 
...
fdr_column <- ifelse(
  ...
  "fdr_ihw", 
  str_subset(colnames(mcols(db_results[[1]])), "fdr")[[1]]
)
...
This stage of the GRAVI workflow takes the results from two individual differential signal analyses and compares the results, by finding peaks which directly overlap and considering the joint behaviour.
In this specific analyses the differential signal response for `r names(pairs)[[1]]` when comparing `r glue_collapse(rev(pairs[[1]]), sep = " Vs ")`, will be integrated with the differential signal response for `r names(pairs)[[2]]` comparing `r glue_collapse(rev(pairs[[2]]), sep = " Vs ")`.
Data will first be described from the perspective of *union peaks* and *treatment peaks*, before moving onto the differential signal obtained in previous steps.

## Integration Of Differential Signal Results

The initial steps of differential signal analysis detect regions with clear changes in signal, however the integration of datasets instead focusses on the *most correct* classification of a given range/window, where unchanged is not simply lack of a statistical result, but a true state needing to be identified.
...
Across two comparisons, this gives 15 possible classifications for each region found with signal from at least one target (e.g. Up-Up, Up-Down, Up-Unchanged, Up-Undetected etc.), given that any potential Undetected-Undetected windows will not be included for obvious reasons.
...
`r if (has_hic) glue("HiC data was also included, with {percent(mean(all_windows$hic), 0.1)} of all windows overlapping a HiC interaction bin. For the {comma(sum(!str_detect(all_windows$status, 'Undetected')))} windows where signal was observed in both datasets, this became {percent(mean(subset(all_windows, !str_detect(status, 'Undetected'))$hic), 0.1)}")`
...
```{r plot-both-donut, fig.cap = "Summary of signal across both comparisons incorporating all sites where signal was detected in either copmarison. Regions where signal is changed in either comparisons are exploded."}
all_windows %>% 
  as_tibble() %>% 
  dplyr::select(ends_with("_status")) %>% 
  mutate(
      across(everything(), str_extract, "Up|Down|Unchanged|Undetected|Ambiguous")
  ) %>% 
  dplyr::rename_with(.cols = everything(), .fn = str_remove_all, "_status") %>% 
  plotSplitDonut(
    ...
    inner_palette = colours$direction %>% 
        unlist() %>% 
        setNames(str_sep_to_title(names(.))),
    outer_palette = colours$direction %>% 
        unlist() %>% 
        setNames(str_sep_to_title(names(.)))
  ) %>% 
    .[[1]]
...
```{r plot-dbwin, fig.cap = glue("*Comparative changes in both ", glue_collapse(comps, sep = " and "), ". The window with most extreme combined change in signal is shown with any mapped genes labelled. The range around zero used for range-based hypothesis testing is indicated with grey dashed lines.*")}
df <- all_windows %>% 
...
```{r plot-dbwin-by-region, fig.cap = glue("*Comparative changes in both ", glue_collapse(comps, sep = " and "), ". The window with most extreme combined change in signal for each separate region is shown with any mapped genes labelled. The range around zero used for range-based hypothesis testing is indicated with grey dashed lines.*")}
df %>% 
  mutate(status = fct_lump_min(status, 2)) %>%
  ggplot() +
  geom_point(
...
```{r plot-dbwin-by-feature, eval = has_features, echo = has_features, fig.cap = glue("*Comparative changes in both ", glue_collapse(comps, sep = " and "), ". The window with most extreme combined change in signal for each separate feature is shown with any mapped genes labelled. The range around zero used for range-based hypothesis testing is indicated with grey dashed lines.*")}
df %>% 
  mutate(status = fct_lump_min(status, 2)) %>%
  ggplot() +
  geom_point(
