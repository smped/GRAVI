...
```{r plot-empty-peaks, eval = length(unique(targets)) == 1, echo = FALSE}
...
samples %>% 
  ...
  fromList() %>% 
  upset(
    sets = seq_along(pairs) %>% 
      vapply(function(x) paste(names(pairs)[[x]], pairs[[x]], sep = "_"), character(2)) %>% 
      as.character() %>% 
      rev() %>% 
    ...
    sets.bar.color = pairs %>% 
      lapply(function(x) treat_colours[x]) %>% 
      unlist() %>% 
      as.character() %>% 
      rev()
  )
...
## classic old R-style with dots instead of spaces. This may need revisting
...
all_windows <- all_windows %>% 
  select(any_of(c("region", "feature")), everything()) %>% 
  mapByFeature(
    all_gr$gene,
    prom = GRangesList(
