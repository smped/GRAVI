...
```{r vioplot-distances, fig.height = 6, fig.cap = "*Distances between windows where maximal signal was detected for each target. The x-axis is truncated at the 99.5^th^ percentile of all observed distances*"}
all_windows %>% 
  as_tibble() %>% 
  ...
  geom_text(
    ...
    nudge_x = 0.02 * max(all_windows$distance, na.rm = TRUE) * (-1),
    size = 3.5
  ) +
  facet_grid(
    n_changed ~ ., scales = "free_y", space= "free_y"
  ...
  coord_cartesian(
    xlim = c(
      0.02 * max(all_windows$distance, na.rm = TRUE) * (-1),
      quantile(all_windows$distance,, na.rm = TRUE, probs =  0.995))
  ) +
  scale_fill_manual(
    values = direction_colours[droplevels(all_windows$status)]
  ) +
...
```{r distance-by-region, fig.height = 6, fig.width = 8, fig.cap = "*Distribution of distances between peaks separated by genomic region. The x-axis is truncated at the 99.5^th^ percentile of all observed distances*"}
all_windows %>% 
...
```{r distance-by-feature, echo = has_features, eval = has_features, fig.height = 6, fig.width = 8, fig.cap = "*Distribution of distances between peaks separated by external feature. The x-axis is truncated at the 99.5^th^ percentile of all observed distances*"}
all_windows %>% 
  filter(!is.na(distance)) %>% 
  as_tibble() %>% 
