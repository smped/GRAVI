...
scale_colour_pw <- scale_colour_manual(
    values = pw_cols, 
    labels = \(x) str_replace_all(
      x,
...
In this specific analyses the differential signal response for `r targets[[1]]` when comparing `r comp_names[[1]]`, will be integrated with the differential signal response for `r targets[[2]]` comparing `r comp_names[[2]]`.
...
if (n_targets > 1) {
  ...
  grid::grid.newpage()
  do.call(VennDiagram::draw.pairwise.venn, venn_args)
}
```
...
A set of `r comma(length(combined_results))` regions with signal was formed by obtaining the union of windows produced during the analysis of `r glue_collapse(unlist(short_names), last = " and ")` individually.
...
```{r plot-pairwise-signal, fig.cap = glue("Pairwise signal levels with boxplots indicating the baseline signal whether signal was detected in both or only the individual comparisons.")}
combined_results %>% 
  as_tibble() %>% 
  ggplot(
    aes(
