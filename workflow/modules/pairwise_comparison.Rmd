...
For this reason, and to minimise issues with hard thresholding, once a window is considered to be of interest, the p-value used in the second comparison is that obtained using the test for $H_0: \mu = 0$ instead of the initial $H_0: 0 < |\mu| < \lambda$.
...
Across two comparisons, this gives 15 possible classifications for each region found with at least one target bound. (e.g. Up-Up, Up-Down, Up-Unchanged, Up-Undetected etc.)
...
The `goseq` method [@YoungGoseq2010] is again used, taking the number of windows mapped to each gene as the source of any sampling bias.
...
A secondary step was then introduced for each window such that if one target was significant and the other target had 1) received an FDR-adjusted p-value < `r fdr_alpha` *using a point-based* H~0~, and 2) had an estimated logFC beyond the range specified for the range-based null hypothesis ($H_0: 0 < |\mu| < \lambda$ for $\lambda = `r round(lambda, 2)`$), these windows were then considered significant in the second comparison.
...
```{r vioplot-window-sizes, fig.cap = "*Combined window sizes for each set of windows*"}
all_windows %>% 
  mutate(w = width) %>% 
  as_tibble() %>% 
  mutate(
...
```{r vioplot-distances, fig.height = 6, fig.cap = "*Distances between windows where maximal signal was detected for each target.*"}
all_windows %>% 
  as_tibble() %>% 
  ...
  ggplot(aes(distance , fct_rev(status))) +
  geom_violin(draw_quantiles = 0.5) +
  facet_grid(
    n_changed ~ ., scales = "free_y", space= "free_y"
  ) +
...
```{r distance-by-region, fig.height = 6, fig.width = 8, fig.cap = "*Distribution of distances between peaks separated by genomic region.*"}
all_windows %>% 
  ...
  geom_text(
    ...
    nudge_x = 0.04 * max(all_windows$distance, na.rm = TRUE) * (-1),
    size = 3.5
  ) +
  scale_fill_manual(
...
```{r distance-by-feature, echo = has_features, eval = has_features, fig.height = 6, fig.width = 8, fig.cap = "*Distribution of distances between peaks separated by external feature.*"}
all_windows %>% 
  filter(!is.na(distance)) %>% 
  as_tibble() %>% 
...
All `r comma(length(mapped_ids))` genes with a mapped binding window from either `r collapseGenes(comps, last = " or ")` were compared to the 'universe' of `r comma(nrow(all_ids))` genes in order to ascertain which key gene-sets were likely to be targeted in either analysis.
...
cp <- htmltools::tags$em(
  glue(
    ...
    ", were compared to all genes to test for enrichment of gene sets ",
    "amongst both individual analyses"
  )
)
tbl <- goseq_mapped_res %>%
  dplyr::select(gs_name, gs_description, pval, adj_p, everything()) %>% 
  reactable(
    filterable = TRUE,
    columns = list2(
      gs_name = colDef(
...
pwf_t1t2 <- structure(
  ...
  nullp(bias.data = all_ids$n_windows, plot.fit = TRUE) 
...
All `r comma(length(intersect(comp_ids[[2]], comp_ids[[1]])))` genes with a mapped binding window to both `r collapseGenes(comps)` were compared to the 'universe' of `r comma(nrow(all_ids))` genes, in order to ascertain whether any key gene-sets were likely to be targeted under *both in combination*.
...
A series of profile heatmaps were generated for any set of pairwise comparisons with more than 10 ranges, and where binding was detected as changed in at least one of the comparisons.
...
grl_to_plot <- all_windows %>% 
  ...
  lapply(
    mutate, 
    centre = rowMeans(cbind(AR_centre, ER_centre), na.rm = TRUE),
    ...
    function(x) {
      x + 
        ...
        scale_colour_manual(
          values = lapply(
            names(pairs), 
            function(x) setNames(treat_colours[pairs[[x]]], paste(x, pairs[[x]]))
          ) %>%
            unlist()
        ) +
        labs(
  ...
  mclapply(
    ...
    function(x) {
      ...
      png(
        ...
        height = 3 + knitr::opts_current$get("fig.height") * n_ranges / 1e3, 
        units = "in", res = 300
      )
