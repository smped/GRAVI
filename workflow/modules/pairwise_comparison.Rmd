...
All `r comma(length(mapped_ids))` genes with a mapped binding window from either `r glue_collapse(targets, " or " )` were compared to the 'universe' of `r comma(nrow(all_ids))` annotated genes in order to ascertain which key gene-sets were likely to be targeted by *either transcription factor*.
...
### Genes Mapped Only to `r targets[[1]]`

```{r goseq_t1_res}
goseq_t1_res <- structure(
  all_ids$gene_id %in% setdiff(target_ids[[1]], target_ids[[2]]),
  names = all_ids$gene_id
) %>% 
  nullp(bias.data = log10(all_ids$w), plot.fit = FALSE) %>% 
  goseq(gene2cat = gs_by_geneid) %>% 
...
All `r comma(length(setdiff(target_ids[[1]], target_ids[[2]])))` genes with a mapped binding window unique to `r targets[[1]]` were compared to the 'universe' of `r comma(nrow(all_ids))` annotated genes in order to ascertain whether any key gene-sets were likely to be targeted *uniquely by* `r targets[[1]]`.
`r if (nrow(goseq_t1_res) == 0) "No enrichment was found."`

```{r tbl-goseq-t1-res, eval = nrow(goseq_t1_res) > 0, echo = nrow(goseq_t1_res) > 0}
cp <- htmltools::tags$em(
  glue(
    "All {comma(length(setdiff(target_ids[[1]], target_ids[[2]])))} genes ", 
    "mapped to a binding window from {targets[[1]]} exclusively ",
    "were compared to all annotated genes to test for enrichment of gene sets ",
...
### Genes Mapped Only to `r targets[[2]]`


```{r goseq_t2_res}
goseq_t2_res <- structure(
  all_ids$gene_id %in% setdiff(target_ids[[2]], target_ids[[1]]),
  names = all_ids$gene_id
) %>% 
  nullp(bias.data = log10(all_ids$w), plot.fit = FALSE) %>% 
  goseq(gene2cat = gs_by_geneid) %>% 
...
All `r comma(length(setdiff(target_ids[[2]], target_ids[[1]])))` genes with a mapped binding window unique to `r targets[[2]]` were compared to the 'universe' of `r comma(nrow(all_ids))` annotated genes in order to ascertain whether any key gene-sets were likely to be targeted *uniquely by* `r targets[[2]]`.
`r if (nrow(goseq_t2_res) == 0) "No enrichment was found."`

```{r tbl-goseq-t2-res, eval = nrow(goseq_t2_res) > 0, echo = nrow(goseq_t2_res) > 0}
cp <- htmltools::tags$em(
  glue(
    "All {comma(length(setdiff(target_ids[[2]], target_ids[[1]])))} genes ", 
    "mapped to a binding window from {targets[[2]]} exclusively ",
    "were compared to all annotated genes to test for enrichment of gene sets ",
    "amongst any unique targets."
  )
)
...
### Genes Mapped to Both `r glue_collapse(targets, " and " )`

```{r goseq_t1t2_res}
goseq_t1t2_res <- structure(
  all_ids$gene_id %in% intersect(target_ids[[2]], target_ids[[1]]),
  names = all_ids$gene_id
) %>% 
  nullp(bias.data = log10(all_ids$w), plot.fit = FALSE) %>% 
  goseq(gene2cat = gs_by_geneid) %>% 
...
All `r comma(length(intersect(target_ids[[2]], target_ids[[1]])))` genes with a mapped binding window to both `r glue_collapse(targets, sep = " and ")` were compared to the 'universe' of `r comma(nrow(all_ids))` annotated genes in order to ascertain whether any key gene-sets were likely to be targeted by *both factors in combination*.
...
cp <- htmltools::tags$em(
  glue(
    "All {comma(length(intersect(target_ids[[2]], target_ids[[1]])))} genes ", 
      ...
      function(x) {
        ...
        htmltools::div(
          htmltools::div(
            ...
            goseq_group_res[[x]] %>% 
              separate(group, into = targets, sep = " - ") %>% 
              nest(gs_name = c("gs_name", "gs_url")) %>% 
              dplyr::select(
                all_of(targets), gs_name, gs_description, pval, fdr, everything()
              ) %>% 
              reactable(
                filterable = TRUE,
                columns = list2(
