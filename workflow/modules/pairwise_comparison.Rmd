...
## Comparison of Genes Mapped To Targets {.tabset}
...
```{r plot-circe-gs, fig.height=min(12, 5 + length(gs) / 3), eval = length(gs) > 0, echo = length(gs) > 0, fig.cap = glue("*Gene-sets considered as enriched in genes from either of the above Venn Diagram subsets. Circle sizes correspond to the number of genes in each subset, with the percentage this represents of each pathway also being presented. Circle fill indicates significance using -log~10~ p-values (unadjusted), with those formally considered enriched indicated with doubleasterisks. Labels are only shown where the number of genes exceeds 5% of the gene set.*")}
list(
  t1t2 = goseq_t1t2_res,
  ...
  mutate(
    ...
    lab = ifelse(enriched, paste0("**", prop), prop)
    # lab = case_when(
  ...
  scale_fill_gradientn(colors = hcl.colors(100, "Reds", rev = TRUE)) +
  guides(size = "none") +
  labs(
