...
```{r target2-by1, fig.cap = glue("*Differential binding patterns of {comps[[2]]} by {comps[[1]]} status. {comps[[2]]} values are shown as points coloured by their individual classification as Up, Down or Unchanged, with the distributions of these shown as boxplots by {comps[[1]]} status in the side panels.*")}
all_windows %>%
  as_tibble() %>% 
  rename_all(
    str_replace_all, pattern = "\\.Vs\\.\\.", replacement = " Vs. "
...
`r if (any_detected) "Regilatory targets were limited to the set of detected genes from the RNA-Seq dataset provided."`
...
if (nrow(goseq_t1t2_res) == 0) para <- glue(para, " No enrichment was found.")
```

`r if (n_targets == 2) para`

```{r tbl-goseq-t1t2-res, eval = nrow(goseq_t1t2_res) > 0 & n_targets == 2, echo = nrow(goseq_t1t2_res) > 0}
goseq_t1t2_res %>%
  dplyr::select(
    gs_name, gs_description, pval, adj_p, mapped, universe, gene_name
  ) %>% 
