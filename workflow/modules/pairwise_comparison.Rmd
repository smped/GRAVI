...
```{r plot-network-t1only, eval = plot_network_t1only, echo = plot_network_t1only, fig.cap=glue("*Network plot showing all gene-sets enriched amongst the overall set of sites with a binding site from __{comps[[1]]} only__.*")}
tg_t1only %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
...
```{r plot-network-t2only, eval = plot_network_t2only, echo = plot_network_t2only, fig.cap=glue("*Network plot showing all gene-sets enriched amongst the overall set of sites with a binding site from __{comps[[2]]} only__.*")}
tg_t2only %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
