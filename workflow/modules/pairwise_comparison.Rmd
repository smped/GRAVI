...
combs <- paste(
  ...
  pull("combn")
direction_colours <- RColorBrewer::brewer.pal(9, "Set1") %>% 
  setNames(str_subset(combs, "Undetected", negate = TRUE))
...
df %>% 
  ...
  scale_colour_manual(
    values = direction_colours[levels(df$status)]
  ) +
  labs(
...
#   as_tibble() %>% 
#   group_by(
#     !!sym(c1_status), !!sym(c2_status), feature
#   ) %>% 
#   summarise(n = dplyr::n(), .groups = "drop_last") %>% 
#   mutate(total = sum(n)) %>% 
#   ungroup() %>% 
...
```{r vioplot-distances, echo = n_targets > 1, eval = n_targets > 1, fig.height = 6, fig.cap = "*Distances between windows where maximal signal was detected for each target. The x-axis is truncated at the 99.5^th^ percentile of all observed distances*"}
...
```{r distance-by-region, echo = n_targets > 1, eval = n_targets > 1, fig.height = 6, fig.width = 8, fig.cap = "*Distribution of distances between peaks separated by genomic region. The x-axis is truncated at the 99.5^th^ percentile of all observed distances*"}
all_windows %>% 
...
```{r distance-by-feature, echo = has_features & n_targets > 1, eval = has_features & n_targets > 1, fig.height = 6, fig.width = 8, fig.cap = "*Distribution of distances between peaks separated by external feature. The x-axis is truncated at the 99.5^th^ percentile of all observed distances*"}
all_windows %>% 
  filter(!is.na(distance)) %>% 
  as_tibble() %>% 
  mutate(feature = fct_infreq(feature)) %>% 
  ggplot(aes(distance, fct_rev(feature), fill = feature)) +
  geom_violin(draw_quantiles = 0.5) +
  geom_text(
    aes(0, fct_rev(feature), label = lab),
    data = . %>% 
      group_by(feature) %>% 
      summarise(n = dplyr::n()) %>% 
      mutate(lab = glue("n = {comma(n)}")),
  ...
  scale_fill_manual(
    values = setNames(feature_colours, str_sep_to_title(names(feature_colours)))
  ) +
  coord_cartesian(xlim = c(- 0.1 * q, q)) +
  labs(
    x = "Distance Between Window Centres (bp)",
...
min_genes <- params$enrichment$min_prop_goseq 
max_genes <- params$enrichment$max_prop_goseq
...
goseq_mapped_res <- tibble(
  gs_name = character(), adj_p = numeric(), enriched = logical()
)
if (mean(pwf_mapped$DEgenes) > min_genes & mean(pwf_mapped$DEgenes) < max_genes) {
...
All `r comma(length(mapped_ids))` genes with a mapped binding window from **either** comparison were compared to the 'universe' of `r comma(nrow(all_ids))` genes in order to ascertain which key gene-sets were likely to be targeted in either analysis.
`r if (any_detected) "Regulatory targets were limited to the set of detected genes from the RNA-Seq dataset provided."`
Gene-length was included as the source of any potential sampling bias for a gene being mapped to a target-bound range.
This analysis provides an overall viewpoint as to which pathways are regulated by `r ifelse(n_targets == 1, unique(names(pairs)), collapseGenes(comps, last = " or "))`.
`r if (sum(goseq_mapped_res$enriched) == 0) "No enrichment was found."`

```{r tbl-goseq-mapped-res, eval = sum(goseq_mapped_res$enriched) > 0, echo = sum(goseq_mapped_res$enriched) > 0}
goseq_mapped_res %>%
...
gs_method <- ifelse(mean(vec_t1t2) > 0.8, "Hypergeometric", "Wallenius")
para <- glue(
  "
  All {comma(length(intersect(comp_ids[[2]], comp_ids[[1]])))} genes with a 
  mapped binding window to **both** {collapseGenes(comps)} were compared to 
  the 'universe' of {comma(length(mapped_ids))} genes mapped to **either**, 
...
if (mean(pwf_t1t2$DEgenes) > min_genes & mean(pwf_t1t2$DEgenes) < max_genes) {
...
if (sum(goseq_t1t2_res$enriched) == 0) para <- glue(para, " No enrichment was found.")
```

`r if (n_targets == 2) para`

```{r tbl-goseq-t1t2-res, eval = sum(goseq_t1t2_res$enriched) > 0 & n_targets == 2, echo = sum(goseq_t1t2_res$enriched) > 0}
goseq_t1t2_res %>%
  dplyr::filter(enriched) %>% 
  dplyr::select(
    gs_name, gs_description, pval, adj_p, mapped, universe, gene_name
...
gs_method <- ifelse(mean(vec_t1only) > 0.8, "Hypergeometric", "Wallenius")
para <- glue(
  "
  All {comma(length(setdiff(comp_ids[[1]], comp_ids[[2]])))} genes with a 
  mapped binding window to **only** {comps[[1]]} were compared to 
  the 'universe' of {comma(length(mapped_ids))} genes mapped to **either** 
  {collapseGenes(comps)}, in order to ascertain whether any key gene-sets 
  were likely to be targeted exclusively.
  ",
  ifelse(
    gs_method == "Hypergeometric",
    "No sampling bias offset was incorporated, instead using a simple Hypergeometric model for enrichment testing",
...
if (mean(pwf_t1only$DEgenes) > min_genes & mean(pwf_t1only$DEgenes) < max_genes) {
...
if (sum(goseq_t1only_res$enriched) == 0) para <- glue(para, " No enrichment was found.")
```

`r if (n_targets == 2) para`

```{r tbl-goseq-t1only-res, eval = n_targets == 2 & sum(goseq_t1only_res$enriched) > 0, echo = sum(goseq_t1only_res$enriched) > 0}
goseq_t1only_res %>%
  dplyr::filter(enriched) %>% 
  dplyr::select(
    gs_name, gs_description, pval, adj_p, mapped, universe, gene_name
...
gs_method <- ifelse(mean(vec_t2only) > 0.8, "Hypergeometric", "Wallenius")
para <- glue(
  "
...
if (mean(pwf_t2only$DEgenes) > min_genes & mean(pwf_t2only$DEgenes) < max_genes) {
...
if (sum(goseq_t2only_res$enriched) == 0) para <- glue(para, " No enrichment was found.")
```

`r if (n_targets == 2) para`

```{r tbl-goseq-t2only-res, eval = n_targets == 2 & sum(goseq_t2only_res$enriched) > 0, echo = sum(goseq_t2only_res$enriched) > 0}
goseq_t2only_res %>%
  dplyr::filter(enriched) %>% 
  dplyr::select(
    gs_name, gs_description, pval, adj_p, mapped, universe, gene_name
...
`r if (sum(goseq_mapped_res$enriched) > 0) "### Visualisation: All Mapped"`

```{r barplot-all-mapped, fig.height=min(15, 5 + 0.15*sum(goseq_mapped_res$enriched)), eval = sum(goseq_mapped_res$enriched) > 0, echo = sum(goseq_mapped_res$enriched) > 0, fig.cap = glue("*Summary of gene sets considered enriched across the set of genes mapped in __either__ comparison. Using {params$enrichment$adj}-adjusted p-values < {enrich_alpha} as indicative of enrichment, all shown gene-sets were considered to be enriched in this analysis, and are presented in order of significance fro top to bottom. The distribution of genes across regions only mapped in one comparison, or mapped in both is also indicated. Asterisk overlays indicate any subsequent analyses where gene-sets were also considered to be enriched.*")}
goseq_mapped_res %>%
  dplyr::select(gs_name, mapped, universe, adj_p) %>%
  ...
  mutate(
    Both = vapply(
      ...
      function(x) {
        ids <- gs_by_gsid[[x]]
        sum(ids %in% comp_ids[[1]] & ids %in% comp_ids[[2]])
      },
      integer(1)
    ),
    t1only = vapply(
  ...
  pivot_longer(
    cols = all_of(c("Both", "t1only", "t2only", "Unmapped")), 
    names_to = "group", values_to = "Total"
  ) %>%
  mutate(
    sig = case_when(
      group == 't1only' & gs_name %in% dplyr::filter(goseq_t1only_res, enriched)$gs_name ~ "*",
      group == 't2only' & gs_name %in% dplyr::filter(goseq_t2only_res, enriched)$gs_name ~ "*",
      group == 'Both' & gs_name %in% dplyr::filter(goseq_t1t2_res, enriched)$gs_name ~ "*",
      TRUE ~ ""
    ),
    group = case_when(
      group == 't1only' ~ paste(comps[[1]], "Only"), 
      group == 't2only' ~ paste(comps[[2]], "Only"), 
      TRUE ~ group
    ),
    group = factor(
      group, 
      levels = c(
        paste(comps[[1]], "Only"), paste(comps[[2]], "Only"), "Both", "Unmapped"
      )
    ),
    gs_name = gs_name %>% 
      fct_inorder() %>% 
      fct_rev() %>% 
      fct_relabel(str_trunc, width = 45)
  ) %>%
  arrange(desc(group)) %>% 
  group_by(gs_name) %>% 
  mutate(y = cumsum(Total)) %>% 
  ungroup() %>% 
  arrange(gs_name, group) %>% 
  ggplot(aes(gs_name, Total, fill = group)) +
  geom_col() +
  geom_point(
    aes(gs_name, y - Total/2),
    data = . %>% 
      dplyr::filter(sig != ""),
    inherit.aes = FALSE, 
    shape = 8, size = 2
  ...
  coord_flip() +
  scale_fill_manual(
    values = c(comp_cols, "Unmapped" = "grey") %>% 
      setNames(str_replace_all(names(.), comps[[1]], paste(comps[[1]], "Only"))) %>% 
      setNames(str_replace_all(names(.), comps[[2]], paste(comps[[2]], "Only"))) 
  ) +
  theme_bw() +
  scale_y_continuous(expand = expansion(c(0, 0.05))) +
  labs(x = "Gene Set", y = "Set Size", fill = "Group")
```

```{r sig-gs}
gs <- c(
  dplyr::filter(goseq_t1only_res, enriched)$gs_name,
  dplyr::filter(goseq_t2only_res, enriched)$gs_name,
  dplyr::filter(goseq_t1t2_res, enriched)$gs_name
)
```

`r if (length(gs) > 0) "### Visualisation: Genes By Group"`

```{r plot-circe-gs, fig.height=min(12, 5 + length(gs) / 3), eval = length(gs) > 0, echo = length(gs) > 0, fig.cap = glue("*Gene-sets considered as enriched in genes from either of the above Venn Diagram subsets. Circle sizes correspond to the number of genes in each subset, with the percentage this represents of each pathway also being presented. Circle fill indicates significance using -log~10~ p-values (unadjusted), with those formally considered enriched indicated with asterisks. The number of asterisks increases with the level of significance, with 3 asterisks indicating an adjusted p-value < 0.001. Labels are only shown where the number of genes exceeds 5% of the gene set.*")}
list(
  t1t2 = goseq_t1t2_res,
  t1only = goseq_t1only_res,
  t2only = goseq_t2only_res
) %>% 
  lapply(dplyr::filter, gs_name %in% gs) %>%
  bind_rows(.id = "group") %>% 
  mutate(
    gs_size = vapply(gs_by_gsid[gs_name], length, integer(1)),
    group = case_when(
      group == 't1only' ~ paste(comps[[1]], "Only"), 
      group == 't2only' ~ paste(comps[[2]], "Only"),
      group == "t1t2"~ "Both",
    ...
    gs_name = gs_name %>% 
      fct_inorder() %>% 
      fct_rev() %>% 
      fct_relabel(str_trunc, width = 50),
    prop = percent(mapped / gs_size, 1),
    lab = case_when(
      adj_p < 0.001 ~ paste0("***", prop),
      adj_p < 0.01 ~ paste0("**", prop),
      adj_p < 0.05 ~ paste0("*", prop),
      TRUE ~ prop
    )
  ) %>%
  ggplot(
    aes(group, gs_name, size = mapped, fill = -log10(pval))
  ) +
  geom_point(pch = 21, stroke = 0.8) +
  geom_label(
    aes(group, gs_name, label = lab),
    data = . %>% 
      dplyr::filter(mapped / gs_size > 0.05),
    inherit.aes = FALSE, size = 4, alpha = 0.6
  ) +
  scale_size_continuous(range = c(0, 200 / length(gs))) +
  scale_fill_viridis_c(option = "inferno") +
  # scale_fill_gradientn(colors = hcl.colors(100, "Inferno", rev = FALSE)) +
  guides(size = "none") +
  labs(
    x = "Enrichment Test Subset", y = "Gene Set", 
    fill = expression(paste("-", log[10], "p"))
  )
```
...
goseq_group_res <- group_ids %>% 
  mclapply(
    function(x) {
      ...
      gs_method <- ifelse(mean(vec) > 0.8, "Hypergeometric", "Wallenius")
      if (mean(pwf$DEgenes) > min_genes & mean(pwf$DEgenes) < max_genes) {
