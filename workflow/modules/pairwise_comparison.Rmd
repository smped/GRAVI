...
rng_list %>%
  ...
  UpSetR::upset(
    ...
    set_size.scale_max = oracle_peaks %>%
      ...
      max() %>%
      multiply_by(1.15),
    sets.bar.color = samples %>%
      distinct(target, treat) %>%
      mutate_all(as.character) %>%
      mutate(
        grp = glue("{target} ({treat})"), 
        colour = treat_colours[treat] %>% setNames(grp)
      ) %>% 
      dplyr::filter(grp %in% names(rng_list)) %>%
      arrange(rev(grp)) %>% 
      pull(colour) 
  )
```
...
if (sum(vec_t1t2) > 0) {
  goseq_t1t2_res <- data.frame(DEgenes = vec_t1t2) %>% 
...
if (sum(vec_t1only) > 0) {
  goseq_t1only_res <- data.frame(DEgenes = vec_t1only) %>% 
    goseq_hyper(gene2cat = gs_by_geneid) %>% 
    dplyr::filter(numDEInCat > 0) %>% 
    mutate(
      adj_p = p.adjust(pval, adj_method),
...
if (sum(vec_t2only) > 0) {
  goseq_t2only_res <- data.frame(DEgenes = vec_t2only) %>% 
    goseq_hyper(gene2cat = gs_by_geneid) %>% 
    dplyr::filter(numDEInCat > 0) %>% 
    mutate(
      adj_p = p.adjust(pval, adj_method),
...
`r if(length(goseq_group_sig) > 0) "### Result Tables {.tabset}"`
