...
tg_mapped <- make_tbl_graph(goseq_mapped_res, gs = gs_by_gsid)
...
tg_mapped %>% 
  ...
  scale_size_continuous(
    range = c(1, 10), breaks = seq(0, 100, by = 25), limits = c(10, 100)
  ) +
...
tg_t1t2 <- make_tbl_graph(goseq_t1t2_res, gs = gs_by_gsid)
...
`r if (plot_network_t1t2) "### Network Plot: Genes Mapped to Both"`

```{r plot-network-t1t2, eval = plot_network_t1t2, echo = plot_network_t1t2, fig.cap=glue("*Network plot showing all gene-sets enriched amongst the overall set of sites with a binding site for __both__ {comps[[1]]} and {comps[[2]]}. Distances were calculated using 1 - the overlap-coefficient with node-pair distances > 0.9 excluded from the final set of edges. Edge width corresponds to the number of shared genes between the two gene-sets.*")}
tg_t1t2 %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
...
tg_t1only <- make_tbl_graph(goseq_t1only_res, gs = gs_by_gsid)
...
`r if (plot_network_t1only) glue("### Network Plot: Genes Mapped to {comps[[1]]} Only"`

```{r plot-network-t1only, eval = plot_network_t1only, echo = plot_network_t1only, fig.cap=glue("*Network plot showing all gene-sets enriched amongst the overall set of sites with a binding site for __{comps[[1]]} only__. Distances were calculated using 1 - the overlap-coefficient with node-pair distances > 0.9 excluded from the final set of edges. Edge width corresponds to the number of shared genes between the two gene-sets.*")}
tg_t1only %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
...
tg_t2only <- make_tbl_graph(goseq_t2only_res, gs = gs_by_gsid)
...
`r if (plot_network_t2only) glue("### Network Plot: Genes Mapped to {comps[[1]]} Only"`

```{r plot-network-t2only, eval = plot_network_t2only, echo = plot_network_t2only, fig.cap=glue("*Network plot showing all gene-sets enriched amongst the overall set of sites with a binding site for __{comps[[2]]} only__. Distances were calculated using 1 - the overlap-coefficient with node-pair distances > 0.9 excluded from the final set of edges. Edge width corresponds to the number of shared genes between the two gene-sets.*")}
tg_t2only %>% 
  ggraph(layout = "fr", weights = 1/d^2) +
  geom_edge_link(aes(width = d, alpha = d)) +
  geom_node_point(
...
goseq_mapped_res %>%
  dplyr::select(gs_name, mapped = numDEInCat, universe = numInCat, adj_p) %>%
...
goseq_group_res <- group_ids %>% 
  ...
  unnest(everything()) %>% 
  split(.$group)
tg_group_res <- goseq_group_res %>% 
  mclapply(make_tbl_graph, gs = gs_by_gsid, mc.cores = threads) %>% 
...
`r if(length(goseq_group_res) > 0) "### Result Tables {.tabset}"`
...
`r if(length(goseq_group_res) > 0) "### Network Plots {.tabset}"`
    ...
    mclapply(
      function(x) {
        ...
        cp <- htmltools::tags$em(
          glue(
            "
            Network plot showing all enriched pathways mapped to genes 
            associated with {x}. 
            "
          )
        )
