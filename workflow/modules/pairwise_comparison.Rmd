...
macs2_path <- here::here(config$paths$macs2, targets)
```
...
oracle_peaks <- file.path(macs2_path, "oracle_peaks.rds") %>%
  lapply(read_rds) %>% 
  setNames(basename(dirname(macs2_path)))
...
all_windows <- db_results %>%
  ...
  mutate(
    ...
    ## Change the classification for unchanged sites to DB if the other target 
    ## is DB, and the FDR using mu0 = 0 is < fdr_alpha + |logFC| > log2(lambda)
    "{comps[[1]]}_status" := case_when(
      
      (!!sym(glue("{comps[[2]]}_fdr")) < fdr_alpha) &
        # (!!sym(glue("{comps[[1]]}_fdr")) < fdr_alpha * 3) &
        (!!sym(glue("{comps[[1]]}_fdr_mu0")) < fdr_alpha) &
        (!!sym(glue("{comps[[1]]}_logFC")) > lambda) ~ "Up",
      
      (!!sym(glue("{comps[[2]]}_fdr")) < fdr_alpha) &
        # (!!sym(glue("{comps[[1]]}_fdr")) < fdr_alpha * 3) &
        (!!sym(glue("{comps[[1]]}_fdr_mu0")) < fdr_alpha) &
        (!!sym(glue("{comps[[1]]}_logFC")) < -1 * lambda) ~ "Down",
    ...
    "{comps[[2]]}_status" := case_when(
      ...
      (!!sym(glue("{comps[[1]]}_fdr")) < fdr_alpha) &
        ...
        (!!sym(glue("{comps[[2]]}_fdr_mu0")) < fdr_alpha) &
        (!!sym(glue("{comps[[2]]}_logFC")) > lambda) ~ "Up",
      
      (!!sym(glue("{comps[[1]]}_fdr")) < fdr_alpha) &
        # (!!sym(glue("{comps[[2]]}_fdr")) < fdr_alpha * 3) &
        (!!sym(glue("{comps[[2]]}_fdr_mu0")) < fdr_alpha) &
        (!!sym(glue("{comps[[2]]}_logFC")) < -1 * lambda) ~ "Down",
...
A set of common windows was formed by obtaining the union of windows produced during the analysis of `r glue_collapse(comps, sep = ", ", last = " and ")` individually.
...
```{r all-out}
out_path <- here::here("output", paste(targets, collapse = "_"))
