...
`r pander(macs2_params)`

with the additional specification of identifying *narrow peaks*.
...
For individual samples, any sample with > `r peak_qc_params$outlier_threshold`x the median number of peaks found across all samples within that treatment group would be marked as failing QC and excluded from further analysis.
Any samples with no detected peaks would also be `r ifelse(peak_qc_params$allow_zero, 'retained, to account for any cytoplasmic-nuclear shift in response to treatment', 'excluded')`.
...
Any peak which was not found in >`r percent(peak_qc_params$min_prop_reps)` of *individual samples* within that treatment group was then discarded.

Peaks which overlapped any blacklisted or greylisted regions were also discarded.
...
htmltools::tagList(
  lapply(
    ...
    function(i) {
      ...
      ## Define the caption
      gr <- join_nearest(grl_to_plot[[i]], gtf_gene, distance = TRUE)
      d <- gr$distance
      gn <- gr$gene_name
      peak_desc <- ifelse(
        nm == "union",
...
As a counter to the highly ranked peaks, coverage for a small set of low ranked peaks are shown below.
...
These effectively represent the low-probability peaks calls, and it may be assumed that the majority of peaks lie somewhere between the highly-ranked and low-ranked peaks.
...
all_out <- list(
  ...
  renv = file.path(
    here::here("output/envs"), glue("{target}_signal_summary.RData")
  )
)
