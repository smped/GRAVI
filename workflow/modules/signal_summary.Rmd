...
if (has_features) {
  p <- consensus_peaks %>% 
    ...
    geom_label(
      ...
      data = . %>% 
        dplyr::filter(n > 0.0295 * length(consensus_peaks)) %>% 
        mutate(
  ...
  lapply(
    ...
    function(i) {
      ...
      plotHFGC(
        ...
        coverage = bwfl, linecol = line_col,
        collapseTranscripts = collapseTrans, maxTrans = 15,
        cytobands = bands_df, zoom = 30, ylim = y_lim,
        col.title = "black", background.title = "white", 
        rotation.title = 90, showAxis = FALSE, fontsize = 10
      )
      dev.off()
        ...
        \(i) {
          ...
          tbl <- consensus_peaks %>% 
            ...
            mutate(p = map_dbl(
              seq_len(nrow(.)), 
              \(i) fisher.test(
                bind_rows(.[i,], no_peak)[c("detected", "undetected")])$p.value
            ),
            ...
            reactable(
              ...
              columns = list(
                region = colDef("Region"),
                feature = colDef("Feature"),
                total = colDef(
...
tbl <- localz_df %>% 
  ...
  reactable(
    showPageSizeOptions = TRUE, searchable = TRUE,
    columns = list(
