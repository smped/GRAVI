...
The strategy for identification of genomic regions with detectable `r target` signal (i.e. peaks), was to run `macs2 calpeak` [@Zhang18798982] on each individual sample, as well as on all samples within each treatment group (`r pander(treat_levels)`).
...
Where required, this were converted to BigWig files in order to save storage space & speed up analysis.

### Individual Sample QC settings

For individual samples, any sample with > `r peak_qc_params$outlier_threshold`x the median number of peaks found across all samples within that treatment group were marked as failing QC and excluded from further analysis.
Any samples with no detected peaks was `r ifelse(peak_qc_params$allow_zero, 'retained', 'excluded')`.

### Filtered Peaks

A set of *filtered peaks* was defined within each treatment group, based on the peaks identified when merging samples.
