...
individual_peaks %>%
  ...
  as_tibble() %>% 
  mutate(qValue = map_dbl(qValue, median)) %>% 
  bind_cols(
    imap(
      valid_sets, 
      \(cols, name) transmute(., "{name}" := if_all(all_of(cols)))
    )
  ) %>% 
  mutate(
    highlight = if_all(names(valid_sets))
  ) %>% 
  simpleUpSet(
    ...
    intersect_layers = default_intersect_layers(
      name = "Peaks in Intersection\n",
      expand = c(0, 0.1)
    ),
    grid_layers = default_grid_layers(
      colour = "highlight", name = "Intersection",
      scale_colour_manual(values = c("grey23", "darkorange"))
    ),
    annotations = list(
