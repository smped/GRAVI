...
if (is.null(all_files)) {
  ...
  ## If no features are provided, write an empty GRL & exit
  all_features <- GRangesList()
  seqinfo(all_features) <- sq
  if (!length(all_features)) 
    cat_time("No features provided. An empty object will be exported")
