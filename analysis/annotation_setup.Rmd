...
if (!file.exists(all_out$gene_features)) {
  ...
  ## This is a very slow process. Currently takes ~10 mins using 18 cores
  red_exons <- all_gr$exon %>% 
    splitAsList(f = .$gene_id) %>% 
    reduce() %>%
    .[unlist(mclapply(., function(x) length(x) > 1, mc.cores = n_cores))]
  split_genes <- all_gr$gene %>% 
    subset(gene_id %in% names(red_exons)) %>% 
    splitAsList(f = .$gene_id) 
  introns <- names(red_exons) %>% 
    mclapply(
      function(x) setdiff(split_genes[[x]], red_exons[[x]]),
      mc.cores = n_cores
    ) %>% 
    setNames(names(red_exons)) %>% 
    .[unlist(mclapply(., function(x) length(x) > 0, mc.cores = n_cores))]
  first_index <- introns %>% 
    mclapply(
  ...
  gene_features$first_intron <- mcmapply(
    function(x, y) {
      setdiff(x[y], c(gene_features$promoters, gene_features$upstream))
    },
    introns, 
    first_index$first,
    mc.cores = n_cores
    ...
    sort()
  gene_features$other_intron <- mcmapply(
    function(x, y) {
      setdiff(x[-y], c(gene_features$promoters, gene_features$upstream))
    }, 
