...
if (!file.exists(all_out$gene_features)) {
  ...
  gene_features$upstream <- all_gr$transcript %>% 
    promoters(upstream = prom$distal, downstream = 0)  %>% 
    setdiffMC(gene_features$promoters, ignore.strand = TRUE) %>% 
    reduceMC() %>% 
    mutate(type = glue("upstream (<{prom$distal/1e3}kb)"))
  ...
  gene_features$upstream <- gene_features$upstream %>%
    ...
    unlist() %>% 
    setNames(c())
  gene_features$exons <- all_gr$exon %>% 
    unstrand() %>% 
    reduceMC() %>% 
    setdiffMC(
      lapply(gene_features[c("promoters", "upstream")], granges) %>% 
        GRangesList() %>% 
        unlist
    ) %>% 
    mutate(type = "exon")
  gene_features$introns <- all_gr$gene %>% 
    unstrand() %>% 
    setdiffMC(
      lapply(gene_features, granges) %>% 
        GRangesList() %>% 
        unlist()
    ) %>% 
    reduceMC() %>% 
    mutate(type = "intron")
  gene_features$distal <- sq %>% 
    GRanges() %>% 
    setdiff(
      suppressWarnings(
        all_gr$gene %>% 
    ...
    mutate(type = "intergenic (>10kb)")
  gene_features$proximal <- setdiff(
    GRanges(sq),
    lapply(gene_features, granges) %>% 
      GRangesList() %>% 
      unlist()
  ...
  ## factor-levels
  o <- c(
    "tss", "promoters", "upstream", "exons", "introns", "proximal",  "distal"
  )
  gene_features <- gene_features[o]
...
4. Exons
5. Introns
6. Intergenic regions within 10kb of a gene
7. Intergenic regions >10kb from any defined genes
...
- *TSS* regions were the individual start sites for each transcript. Given that many transcripts can originate from thesame TSS, this is somewhat smaller than the number of actual transcripts defined in the GTF
...
gene_features %>% 
  lapply(
    function(x){
      tibble(n = length(x), width = sum(width(x)), type = unique(x$type))
    }
  ) %>%
  bind_rows() %>% 
  mutate(
  ...
  dplyr::select(
    Type, N, `Total Width (Mb)` = Width,
    `Average Width (Kb)` = Mn, `% Genome`
  ) %>% 
  pander(
    justify = "lrrrr",
    caption = "Summary of unique features defined above"
  )
```
...
```{r pie-gene-features, fig.height=8, fig.width=8, fig.cap = "Summary of gene-derived features using the above workflow and the supplied GTF. Blacklisted regions were not considered for this step of the annotation."}
gene_features %>% 
  lapply(
    function(x){
      tibble(n = length(x), width = sum(width(x)), type = unique(x$type))
  ...
  bind_rows() %>% 
  dplyr::filter(type != "tss") %>% 
  mutate(
    width = width / 1e6,
    mn = width/n,
    type = str_to_title(type) %>% fct_inorder(),
    p = width / sum(width),
    lab = glue("{percent(p, 0.1)}")
  ) %>% 
  arrange(desc(type)) %>% 
  mutate(
    y = cumsum(width) - 0.5*width
  ) %>% 
  ggplot(aes(1, width, fill = type)) +
...
```{r plot-example, fig.cap = paste("12kb region surrounding", collapseGenes(subset(all_gr$gene, gene_id == id)$gene_name), "showing all annotated regions.")}
...
if (!is.null(feat_path)) {
  ...
  mcols(external_features) <- cbind(
    ...
    gene_features %>% 
      lapply(function(x) overlapsProp(external_features, x)) %>% 
      as("DataFrame")
  )
}
```
...
```{r plot-feature-by-region, eval = !is.null(external_features), echo = !is.null(external_features), fig.cap = "The proportion of the ranges provided as external annotations which overlap the gene-centric features defined previously."}
regions <- vapply(gene_features, function(x){unique(x$type)}, character(1))
external_features %>% 
  mutate(w = width) %>% 
  mcols() %>% 
  as.data.frame() %>% 
  as_tibble() %>% 
  dplyr::mutate(
    across(.cols = all_of(names(regions)), function(x) x*w)
  ) %>% 
  group_by(feature) %>% 
  summarise(
    across(all_of(c(names(regions), "w")), sum)
  ) %>% 
  pivot_longer(cols = all_of(names(regions)), names_to = "region", values_to = "total") %>% 
...
## USE ALPHA TO INDICATE DETECTED/UNDETECTED & REPLICATE THE ABOVE PLOT
