...
if (!file.exists(all_out$gene_regions)) {
  ...
  gene_regions$upstream <- all_gr$transcript %>% 
    ...
    reduceMC() %>% 
    mutate(
      type = glue("upstream (<{prom$distal/1e3}kb)"),
      detected = vapply(detected, any, logical(1))
    )
...
region_cols <- c(
  "black", 
  hcl.colors(length(gene_regions) - 1, rev = TRUE, alpha = 0.7)
)
names(region_cols) <- gene_regions %>% 
  lapply(select, type) %>% 
  vapply(function(x) unique(x$type), character(1))
...
```{r pie-gene-regions, fig.height=8, fig.width=8, fig.cap = "Summary of gene-cetric regions using the above workflow and the supplied GTF. Blacklisted regions were not considered for this step of the annotation."}
gene_regions %>% 
  lapply(
    function(x){
...
if (!is.null(feat_path)) {
  ...
  mcols(external_features) <- cbind(
    ...
    gene_regions %>% 
      lapply(
        function(x) overlapsProp(external_features, subset(x, detected))
      ) %>% 
      as("DataFrame") %>% 
...
```{r plot-feature-by-region, eval = !is.null(external_features), echo = !is.null(external_features), fig.cap = "The proportion of the external features which overlap the gene-centric regions defined previously. Regions associated with detected genes are shown as solid colours, whilst those with no information are shown as semi-transparent colours."}
...
suppressWarnings(
  external_features %>% 
    ...
    dplyr::filter(!str_detect(region, "tss")) %>% 
    ...
    mutate(
      ...
      Feature = str_sep_to_title(feature) %>% 
        paste("(External)", sep = "\n"),
      region = str_to_title(regions[region]) %>% 
