...
A set of genomic features was defined for comparison against binding of all ChIP targets.
These included:

- Promoters
    + Within 2kb of the TSS
    + <5kb upstream of the TSS
- Gene Body
    + Exons
    + The 1st intron (excluding TSS + 2kb)
    + Other introns
- Intergenic
    + Within 5-10kb
    + Within 10-100kb 
...
In addition to these genomic regions, the transcription start sites (TSS) were also defined for each transcript.

```{r}
# Very, very slow (8.5 mins with 18 cores) but retains correct gene ids
multi_exon_ids <- tibble(gene_id = all_gr$exon$gene_id) %>% 
  group_by(gene_id) %>% 
  summarise(n = dplyr::n()) %>% 
  dplyr::filter(n > 1)
introns <- mcmapply(
  ...
  all_gr$gene %>% 
    filter(gene_id %in% multi_exon_ids$gene_id) %>% 
    split(.$gene_id),
  all_gr$exon %>% 
    filter(gene_id %in% multi_exon_ids$gene_id) %>% 
    split(.$gene_id),
...
first_index <- introns %>% 
  mclapply(
    function(x) list(tibble(strand = unique(strand(x)), n = length(x))),
    mc.cores = n_cores
  ) %>% 
  as_tibble() %>% 
  pivot_longer(cols = everything(), names_to = "gene_id") %>% 
  unnest(value) %>% 
  mutate(first = ifelse(strand == "+", 1, n))
stopifnot(all(first_index$gene_id == names(introns)))
first_intron <- mcmapply(
  function(x, y) x[y], 
  introns, 
  first_index$first,
  mc.cores = n_cores
  ...
  sort()
other_introns <- mcmapply(
  function(x, y) x[-y], 
  introns, first_index$first,
  mc.cores = n_cores
) %>% 
  GRangesList() %>% 
  unlist() %>% 
...
gene_features <- list(
  ...
  promoters = promoters(all_gr$transcript, upstream = 2e3, downstream = 2e3),
  upstream = all_gr$transcript %>% 
    promoters(upstream = 5e3, downstream = 0) %>% 
    resize(width = 3e3, fix = "start"),
  exons = all_gr$exon,
  first_intron = first_intron,
  other_intron = other_introns
)
```
