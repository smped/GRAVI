...
## Make a pie chart based on genomic width & number of regions
gene_features %>% 
  lapply(
    function(x){
  ...
  geom_label(
    aes(y = y, label = percent(p, 0.1)), 
    size=5, fill = "white", alpha = 0.5
  ) +
...
```{r}
gr <- subset(all_gr$gene, gene_name == "GATA3") %>% 
  resize(width = width(.) + 2e4, fix = 'center') %>% 
  unstrand()
ft <- gene_features %>% 
  lapply(subsetByOverlaps, gr) 
ft$intergenic <- all_gr$gene %>%
  unstrand() %>% 
  subsetByOverlaps(gr) %>%
  resize(width = width(.) + 2e4, fix = 'center') %>%
  reduce() %>% 
  ...
  mutate(type = "Intergenic (<10kb)")
ft$distal <- all_gr$gene %>%
  unstrand() %>% 
  subsetByOverlaps(gr) %>%
  resize(width = width(.) + 2.5e4, fix = 'center') %>%
  reduce() %>% 
  setdiff(
    lapply(ft, granges) %>%
      GRangesList() %>%
...
df %>% 
  ...
  labs(
    x = "BP", y = "Transcript", fill = "Feature"
  ) +
  theme(
