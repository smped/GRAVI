...
All genes with a binding window from either `r glue_collapse(targets, " or " )` were compared to the 'universe' of annotated genes in order to ascertain which key gene-sets were likely to be targeted by either transcription factor.
...
cp <- htmltools::tags$em(
  glue(
    "Genes mapped to a binding window from either ",
    glue_collapse(targets, " or " ),
...
<!-- This may be trickier as we have so many groups -->

```{r}
goseq_group_res <- tibble()
...
```{r}
structure(
  mapped_ids %in% setdiff(target_ids[[2]], target_ids[[1]]),
  names = mapped_ids
) %>% 
  nullp(bias.data = log10(gene_lengths), plot.fit = FALSE) %>% 
  goseq(gene2cat = gs_by_geneid) %>% 
  dplyr::filter(numDEInCat > 0) %>% 
  ...
  mclapply(
    function(x) {
      ...
      res <- goseq(pwf, gene2cat = gs_by_geneid) %>% 
        as_tibble() %>% 
        dplyr::filter(numDEInCat > 0) %>% 
        mutate(fdr = p.adjust(res$over_represented_pvalue, "fdr")) %>% 
        dplyr::select(
