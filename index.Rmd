...
This workflow is managed using `snakemake` (>v7.7) for running locally or on any HPC, and is designed to minimally take one ChIP target under at least two conditions.
...
- A gtf with gene, transcript and exon-level annotations. The workflow has been tested extensively with [Gencode annotations](https://www.gencodegenes.org/) and these are preferred.
- A blacklist should also be provided as a bed file. These can easily be obtained from [here](https://github.com/Boyle-Lab/Blacklist/tree/master/lists)


In addition to $\geq 1$ ChIP targets, **optional** input includes:

- Differential Expression results from a single RNA-Seq experiment as a `tsv/csv`
