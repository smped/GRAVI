...
- `macs2:` (Default: "output/macs2") Output from `macs2 callpeak` will be placed in this directory, mirroring the input structure from `data/aligned` where each ChIP target (e.g. TF1, TF2) will have results written to separate directories.
- `bigwig:` (Default: "output/bigwig") After running `macs2 callpeak`, `bedGraph` files will be converted to the more space efficient `bigwig` files. The directory structure from both `data/aligned` and `output/macs2` will again be mirrored such that each ChIP target has all samples written to the same directory
